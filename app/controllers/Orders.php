...
//        }
    ...
    public function index($warehouse_id = null)
    {
        ...
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => '#', 'page' => lang('orders_pending')));
        $meta = array('page_title' => lang('orders_pending'), 'bc' => $bc);
    ...
    public function getSales($year, $month, $warehouse_id = null)
    {
        ...
        if ($this->Supplier) {
            ...
            if ($warehouse_id) {
                $this->datatables
                        ->select($this->db->dbprefix('sales').".id as id, DATE_FORMAT({$this->db->dbprefix('sales')}.date, '%Y-%m-%d %T') as date, {$this->db->dbprefix('sales')}.reference_no, {$this->db->dbprefix('sales')}.biller, {$this->db->dbprefix('sales')}.customer, {$this->db->dbprefix('sales')}.sale_status, {$this->db->dbprefix('sales')}.grand_total, {$this->db->dbprefix('sales')}.paid, ({$this->db->dbprefix('sales')}.grand_total-{$this->db->dbprefix('sales')}.paid) as balance, {$this->db->dbprefix('sales')}.payment_status, {$this->db->dbprefix('sales')}.attachment, {$this->db->dbprefix('sales')}.return_id")
                        ->from('sales');
                $this->datatables->where('warehouse_id', $warehouse_id);
            } else {
                $this->datatables
                    ->select("id as id, DATE_FORMAT(date, '%Y-%m-%d %T') as date, reference_no, biller, customer, sale_status, grand_total, paid, (grand_total-paid) as balance, payment_status, attachment, return_id")
                    ->from('sales');
            }
            $this->datatables->where('sales.biller_id', $this->session->userdata('company_id'));
...
//        $this->datatables->where('sale_status', 'returned');
        ...
        $this->datatables->where('sale_status', 'pending');
    ...
    public function view($id = null)
    {
        ...
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('sales'), 'page' => lang('orders_pending')), array('link' => '#', 'page' => lang('view')));
...
//        if ($this->form_validation->run() == true && $this->sma->send_email($to, $subject, $message, null, null, $attachment, $cc, $bcc)) {
        ...
        try {
            ...
            if ($this->form_validation->run() == true) {
                ...
                if ($_FILES['document']['size'] > 0) {
                    ...
                    $file               = $this->integration_model->upload_files($_FILES['document']);
                    $photo              = $file->url;
    ...
    //        echo json_encode($data);echo json_encode($products);echo json_encode($payment);die();
            if ($this->form_validation->run() == true && $this->sales_model->addSale($data, $products, $payment)) {
                ...
                $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('sales'), 'page' => lang('orders_pending')), array('link' => '#', 'page' => lang('add_sale')));
        ...
        try {
           ...
           // $inv = 0;
            ...
            if ($this->form_validation->run() == true && $this->sales_model->updateSale($id, $data, $products)) {
                ...
                $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('sales'), 'page' => lang('orders_pending')), array('link' => '#', 'page' => lang('edit_sale')));
    ...
    public function return_sale($id = null)
    {
        ...
        try {
            ...
            if ($sale->return_id) {
                throw new \Exception(lang("sale_already_returned"));
    ...
    //        echo json_encode($_POST);die();
            if ($this->form_validation->run() == true && $this->sales_model->addSale($data, $products, $payment, $si_return)) {
                ...
                $this->data['tax_rates'] = $this->site->getAllTaxRates();
                $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('sales'), 'page' => lang('orders_pending')), array('link' => '#', 'page' => lang('return_sale')));
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (!empty($_POST['val'])) {
                if ($this->input->post('form_action') == 'delete') {
                    ...
                    $sheet->setTitle(lang('orders_pending'))
                          ...
                          ->SetCellValue('F1', lang('paid'))
                          ->SetCellValue('G1', lang('payment_status'));
                    ...
                    foreach ($_POST['val'] as $id) {
                        ...
                        $sheet->SetCellValue('A' . $row, $this->sma->hrld($sale->date))
                              ...
                              ->SetCellValue('D' . $row, $sale->customer)
                              ->SetCellValue('E' . $row, $sale->grand_total)
                              ->SetCellValue('F' . $row, lang($sale->paid))
                              ->SetCellValue('G' . $row, lang($sale->payment_status));
                        ...
                        foreach($items as $item){
                            ...
                            $sheet->SetCellValue('A' . $row_item, $this->sma->hrld($sale->date))
                                  ...
                                  ->SetCellValue('E' . $row_item, $this->sma->formatQuantity($item->quantity))
                                  ->SetCellValue('F' . $row_item, $this->sma->formatMoney($item->unit_price))
                                  ->SetCellValue('G' . $row_item, $this->sma->formatDecimal($item->item_tax))
    ...
    public function deliveries()
    {
        ...
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('sales'), 'page' => lang('orders_pending')), array('link' => '#', 'page' => lang('deliveries')));
    ...
    public function add_delivery($id = null)
    {
        ...
        try {
            ...
            if ($delivery = $this->sales_model->getDeliveryBySaleID($id)) {
                ...
                if ($this->form_validation->run() == true && $this->sales_model->addDelivery($dlDetails, $shipping_cost)) {
                    $this->db->trans_commit();
    ...
    //                $coor_customer=$this->sma->get_coordinates($data_customer->city,$data_customer->address,$data_customer->state);
    //                $coor_user=$this->sma->get_coordinates($data_user->city,$data_user->address,$data_user->state);
    //                $range=$this->sma->GetDrivingDistance($coor_user['lat'],$coor_customer['lat'],$coor_user['long'],$coor_customer['long']);
    //                $space=substr_replace((string)$range['distance'],'',strpos((string)$range['distance'],"."));
    //                $cost=$this->site->getShippingChargesData(intval($space));
    ...
    //        $this->sma->transactionPermissions('deliveries',$id);
            ...
            if ($this->form_validation->run() == true && $this->sales_model->updateDelivery($id, $dlDetails, $shipping_cost)) {
                $this->db->trans_commit();
            ...
            if (!empty($_POST['val'])) {
                ...
                if ($this->input->post('form_action') == 'export_excel' || $this->input->post('form_action') == 'export_pdf') {
                    ...
                    $sheet->SetCellValue('A1', lang('date'))
                          ->SetCellValue('B1', lang('do_reference_no'))
                    ...
                    foreach ($_POST['val'] as $id) {
                        ...
                        $sheet->SetCellValue('A' . $row, $this->sma->hrld($delivery->date))
                              ->SetCellValue('B' . $row, $delivery->do_reference_no)
                              ->SetCellValue('C' . $row, $delivery->sale_reference_no)
                              ->SetCellValue('D' . $row, $delivery->customer)
                              ->SetCellValue('E' . $row, $delivery->address)
                              ->SetCellValue('F' . $row, lang($delivery->status));
                        $row++;
                    }
    ...
    public function gift_cards()
    {
        ...
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('sales'), 'page' => lang('orders_pending')), array('link' => '#', 'page' => lang('gift_cards')));
    ...
    public function sale_by_csv()
    {
        ...
        if ($this->form_validation->run() == true && $this->sales_model->addSale($data, $products, $payment)) {
            ...
            $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('sales'), 'page' => lang('orders_pending')), array('link' => '#', 'page' => lang('add_sale_by_csv')));
