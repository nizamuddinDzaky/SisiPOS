...
class Quotes extends MY_Controller
{
    ...
    public function getQuotes($warehouse_id = null)
    {
        ...
        $email_link = anchor('quotes/email/$1', '<i class="fa fa-envelope"></i> ' . lang('email_quote'), 'data-toggle="modal" data-target="#myModal"  data-backdrop="static"');
        $edit_link = anchor('quotes/edit/$1', '<i class="fa fa-edit"></i> ' . lang('edit_quote'));
        $convert_link = anchor('sales_booking/add_booking_sale/$1', '<i class="fa fa-heart"></i> ' . lang('create_sale'));
        ...
        $delete_link = "<!-- Sementara tombol delete disembunyikan  <a href='#' class='po' title='<b>" . $this->lang->line("delete_quote") . "</b>' data-content=\"<p>"
        ...
        if ($warehouse_id) {
            $this->datatables
                ->select("quotes.id as id, quotes.date, quotes.reference_no, quotes.biller, quotes.customer, quotes.supplier, quotes.grand_total, quotes.status, quotes.attachment")
                ->from('quotes')
                ->join('warehouses', 'quotes.warehouse_id=warehouses.id', 'left')
    ...
    public function modal_view($quote_id = null)
    {
        ...
        $this->sma->transactionPermissions('quotes', $quote_id);
        $this->data['error'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
    ...
    public function add()
    {
        ...
        try {
            ...
            if ($this->form_validation->run() == true && $this->quotes_model->addQuote($data, $products)) {
                ...
                //nge cek apakah jumlah Quotations telah limit
                $isLimited = $this->authorized_model->isQuoteLimited($this->session->userdata('biller_id'));
                if ($isLimited["status"]) {
                    $message = str_replace("xxx", $isLimited["max"], lang("limited_trx"));
                    $message = str_replace("yyy", lang("quotes"), $message);
    ...
    public function edit($id = null)
    {
        ...
        try {
            ...
            $this->sma->transactionPermissions('quotes', $id);
        ...
        if ($this->form_validation->run() == true) {
            if (!empty($_POST['val'])) {
                if ($this->input->post('form_action') == 'delete') {
                    ...
                    $sheet->setTitle(lang('quotes'));
                    $sheet->SetCellValue('A1', lang('date'))
                          ->SetCellValue('B1', lang('reference_no'))
                          ->SetCellValue('C1', lang('biller'))
                          ->SetCellValue('D1', lang('customer'))
                          ->SetCellValue('E1', lang('total'))
                          ->SetCellValue('F1', lang('status'));

                    $row = 2;
                    foreach ($_POST['val'] as $id) {
                        $qu = $this->quotes_model->getQuoteByID($id);
                        $sheet->SetCellValue('A' . $row, $this->sma->hrld($qu->date))
                              ->SetCellValue('B' . $row, $qu->reference_no)
                              ->SetCellValue('C' . $row, $qu->biller)
                              ->SetCellValue('D' . $row, $qu->customer)
                              ->SetCellValue('E' . $row, $qu->total)
                              ->SetCellValue('F' . $row, $qu->status);
                        $row++;
                    }
