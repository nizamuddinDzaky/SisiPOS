...
class Transfers extends MY_Controller
{
    ...
    public function getTransfers()
    {
        ...
        $detail_link = anchor('transfers/view/$1', '<i class="fa fa-file-text-o"></i> ' . lang('transfer_details'), 'data-toggle="modal" data-target="#myModal"  data-backdrop="static"');
        $email_link = anchor('transfers/email/$1', '<i class="fa fa-envelope"></i> ' . lang('email_transfer'), 'data-toggle="modal" data-target="#myModal"  data-backdrop="static"');
        ...
        $delete_link = "<!-- Sementara tombol delete disembunyikan  <a href='#' class='tip po' title='<b>" . lang("delete_transfer") . "</b>' data-content=\"<p>"
    ...
    public function add()
    {
        ...
        try {
            ...
            if ($this->form_validation->run() == true && $this->transfers_model->addTransfer($data, $products)) {
                ...
                //nge cek apakah jumlah Transfer telah limit
                $isLimited = $this->authorized_model->isTransferLimited($this->session->userdata('company_id'));
                if ($isLimited["status"]) {
                    $message = str_replace("xxx", $isLimited["max"], lang("limited_trx"));
                    $message = str_replace("yyy", lang("transfers"), $message);
        ...
        if ($this->form_validation->run() == true) {
            if (!empty($_POST['val'])) {
                if ($this->input->post('form_action') == 'delete') {
                    ...
                    $sheet->setTitle(lang('transfers'))
                          ->SetCellValue('A1', lang('date'))
                          ->SetCellValue('B1', lang('reference_no'))
                          ->SetCellValue('C1', lang('from_warehouse'))
                          ->SetCellValue('D1', lang('to_warehouse'))
                    ...
                    foreach ($_POST['val'] as $id) {
                        ...
                        $sheet->SetCellValue('A' . $row, $this->sma->hrld($tansfer->date))
                              ->SetCellValue('B' . $row, $tansfer->reference_no)
                              ->SetCellValue('C' . $row, $tansfer->from_warehouse)
                              ->SetCellValue('D' . $row, $tansfer->to_warehouse)
                              ->SetCellValue('E' . $row, $tansfer->grand_total)
                              ->SetCellValue('F' . $row, $tansfer->status);
                        $row++;
                    }
