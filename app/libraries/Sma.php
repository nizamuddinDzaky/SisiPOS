...
class Sma
{
    ...
    {
        if ($ldate) {
            if (!$this->dateFormats['php_ldate']) {
                $format = 'd/m/Y H:i';
            } else {
                $format = $this->dateFormats['php_ldate'];
            }
    ...
    public function send_email_php_mailer($from, $to, $attachments, $subject, $body)
    {
    ...
    public function send_email($to, $subject, $message, $from = null, $from_name = null, $attachment = null, $cc = null, $bcc = null)
    {
        ...
        $config['useragent'] = "Forca Point Of Sales";
    ...
    public function checkMenuPermissions($action = null, $js = null, $module = null)
    {
        $url = $this->m . '/' . $this->v . '/';
        $this->load->model('menu_model');
        $access = $this->menu_model->getMenuActiveByGroupId($this->session->userdata('group_id'), $url);
        // var_dump($access);die;
        // if ($access) {
        if ($access && $access[0]->is_active == 0) {
            return false;
        }
        return true;
    }
    ...
    {
        if ($this->Owner || $this->Admin || $this->Supplier || $this->LT || $this->Principal) {
    ...
    public function transactionPermissions($tables, $id)
    {
        if (!$this->Owner && !$this->Principal) {
            $this->db->join('warehouses', $tables . '.warehouse_id=warehouses.id', 'left');
            $this->db->where($tables . '.id', $id)->where('warehouses.company_id', $this->session->userdata('company_id'));
    ...
    public function transactionPermissionsDelivery($tables, $id)
    {
        if (!$this->Owner && !$this->Principal && !$this->Admin) {
            $this->db->join('warehouses', $tables . '.warehouse_id=warehouses.id', 'left');
            $this->db->where($tables . '.id', $id)->where('warehouses.company_id', $this->session->userdata('company_id'));
            $q = $this->db->get($tables);
            if ($q->num_rows() > 0) {
    ...
    public function UpdateAutorizedPermissions($tables)
    {
        if (!$this->Owner) {
            $this->db->select($tables);
            $this->db->from('authorized');
            $this->db->join($tables, $this->db->dbprefix($tables).".company_id=authorized.company_id", "left");
            $this->db->where($this->db->dbprefix('authorized').".company_id", $this->session->userdata('company_id'));
            if($tables == 'warehouses'){
                $this->db->where($this->db->dbprefix($tables).".active", null);
            }
            else{
                $this->db->where($this->db->dbprefix($tables).".active", 1);
            }

            $q = $this->db->get(); 
            if ($q->num_rows() < $q->row()->$tables) {
                return TRUE;
            }
            return FALSE;
        }
    ...
    public function CreatedPermissions($tables)
    {
        if (!$this->Owner) {
            $this->db->select($tables);
            $this->db->join($tables, $this->db->dbprefix($tables) . ".company_id=authorized.company_id", "left");
            //            $this->db->where("{$tables}.company_id",$this->session->userdata('company_id'));
            //            $q=$this->db->get($tables);
            $q = $this->db->get_where('authorized', array($this->db->dbprefix('authorized') . ".company_id" => $this->session->userdata('company_id')));
            if ($q->num_rows() < $q->row()->$tables) {
                return TRUE;
            }
            return FALSE;
    ...
    public function qrcode($type = 'text', $text = 'https://pos.forca.id', $size = 2, $level = 'H', $sq = null)
    {
        if ($_SERVER['HTTP_HOST'] == 'pos.forca.id') {
            $file_name = 'assets/uploads/qrcode_prod' . $this->session->userdata('user_id') . ($sq ? $sq : '') . ($this->Settings->barcode_img ? '.png' : '.svg');
        } else {
    ...
    public function logged_as_aksestoko()
    {
        return (bool) ($this->session->userdata('identity') && $this->session->userdata('username') && $this->session->userdata('group_id') == 10 && $this->session->userdata('aksestoko'));
    }
    ...
    public function update_award_points($total, $customer, $user, $scope = null, $used_point = null)
    {
        $point = $this->site->getPoints();
        $company = $this->site->getCompanyByID($customer);
        if ($used_point && $company->company_id != 1) {
            $total_points = $company->award_points - $used_point;
    ...
    {
        die("<script type='text/javascript'>setTimeout(function(){ window.top.location.href = '" . ($page ? base_url($page) : (isset($_SERVER["HTTP_REFERER"]) ? $_SERVER["HTTP_REFERER"] : base_url('welcome'))) . "'; }, 10);</script>");
    }
    ...
    public function paid_opts($paid_by = null, $purchase = false)
    {
        ...
        <option value="bank"' . ($paid_by && $paid_by == 'bank' ? ' selected="selected"' : '') . '>' . lang("bank") . '</option>';
        // ditambah bank untuk mengatasi ketika payment dari AksesToko , karena payment akestoko bertipe bank
    ...
    {
        $url = "https://maps.googleapis.com/maps/api/distancematrix/json?origins=" . $lat1 . "," . $long1 . "&destinations=" . $lat2 . "," . $long2 . "&mode=driving&language=en-EN&key=" . _KEY_MAP;
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, $url);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
        curl_setopt($ch, CURLOPT_PROXYPORT, 3128);
    ...
    {
        if (!$this->Owner) {
            //            $this->db->group_start()->where('status !=',NULL)->or_where('status !=','free')->group_end();
            //            $this->db->where('expired');
            $q = $this->db->get_where('authorized', array('company_id' => $this->session->userdata('company_id')), 1);
            if ($q->num_rows() > 0) {
                return $q->row();
            }
        }
    ...
    public function email_trap($to, $subject, $message, $from = null, $from_name = null, $attachment = null, $cc = null, $bcc = null)
    {
        $this->load->library('email');
        $config['useragent'] = "Forca Point Of Sales";
        ...
        if ($this->Settings->protocol == 'sendmail') {
            ...
            $config['smtp_user'] = '303b12650136da';
            $config['smtp_pass'] = '4a692a2c846ee0';
