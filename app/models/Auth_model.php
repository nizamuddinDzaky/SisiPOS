...
class Auth_model extends CI_Model
{
    ...
    public function validation_password($identity, $old)
    {
        $this->trigger_events('pre_change_password');
        $this->trigger_events('extra_where');
        ...
        if ($old_password_matches === true) {
            ...
            if ($this->curl_->addEcomerce($old) === true); {
                $this->set_message('Add Users Ecomerce successful');
                return true;
            }
        } else {
            $this->set_error('Anda Salah Memasukkan Password');
            return false;
        }

        $this->set_error('Add Users Ecomerce Unsuccessful');
        return false;
    }
    ...
    public function login($identity, $password, $remember = false, $provider = null, $ip_address = null)
    {
        ...
        $query = $this->db->select($this->identity_column . ', username, email, id, password, active, last_login, last_ip_address, avatar, gender, group_id, warehouse_id, biller_id, company_id, view_right, edit_right, allow_discount, show_cost, show_price,company, last_update')
        ...
        if ($query->num_rows() === 1) {
            ...
            if ($password === true || (!empty($identity) && !empty($provider))) {
                ...
                $this->update_last_login_ip($user->id, $ip_address);
                $ldata = array('user_id' => $user->id, 'ip_address' => empty($ip_address) ? $this->input->ip_address() : $ip_address, 'login' => $identity);
    ...
    public function update_last_login_ip($id, $ip_address = null)
    {
        $this->trigger_events('update_last_login_ip');

        $this->trigger_events('extra_where');

        $this->db->update($this->tables['users'], array('last_ip_address' => empty($ip_address) ? $this->input->ip_address() : $ip_address), array('id' => $id));
    ...
    public function set_session($user)
    {
        ...
        $company = $this->companies->getCompanyByID($user->company_id);
    ...
    public function login_remembered_user()
    {
        ...
        //get the user
        $this->trigger_events('extra_where');
        $query = $this->db->select($this->identity_column . ', id, username, email, last_login, last_ip_address, avatar, gender, group_id, warehouse_id, biller_id, company_id, view_right, allow_discount, edit_right, show_cost, show_price, supplier_id, manager_area, last_update')
    ...
    public function checkCF1Distributor($cf1, $company_id)
    {
        $this->db->where('group_name', 'biller');
        $this->db->where('cf1', $cf1);
        $this->db->where('id !=', $company_id);
        $this->db->where('(client_id is null or client_id != "aksestoko")');
        $q = $this->db->get('sma_companies', 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return false;
    ...
    public function insertUser($data, $companies, $plugin = null)
    {
        $this->trigger_events('pre_register');
        ...
        $data['ip_address'] = empty($data['ip_address']) ? $ip_address : $data['ip_address'];
    ...
    public function cancelBillingInv($id, $data)
    {
        if ($this->db->update('billing_invoices', $data, ['id' => $id])) {
            return true;
        }
        return false;
    }

    public function updateBillingInv($data, $addon = null)
    {
        $billing = $this->auth_model->getBillingInv();

        if ($this->db->update('billing_invoices', $data, ['reference_no' => $billing->reference_no, 'company_id' => $data['company_id']])) {
            $billingInv = $this->getBillingInv();
            $this->db->delete('billing_invoice_items', ['billing_invoice_id' => $billingInv->id]);
            if ($addon) {
                foreach ($addon as $item) {
                    $item['billing_invoice_id'] = $billingInv->id;
                    $this->db->insert('billing_invoice_items', $item);
                }
            }
            $this->db->update('authorized', array('status' => 'pending'), array('id' => $data['authorized_id']));
            return true;
        }
        return false;
    }

    public function addBillingInv($data, $addon = null)
    {
        ...
        if ($this->db->insert('billing_invoices', $data)) {
            ...
            if ($addon) {
                foreach ($addon as $k => $item) {
                    $addon[$k]->billing_invoice_id = $billing_invoice_id;
                    $this->db->insert('billing_invoice_items', $item);
                }
            }
            $author = $this->auth_model->getAuthor($data['company_id']); 
            if($author->status == 'activated' && $author->plan_id != '2'){
                $update = $this->db->update('authorized', array('status'=>'pending'), array('id'=>$data['authorized_id']));
            }

            $comp_id = $this->session->userdata('company_id');
            $del_temp = $this->db->delete('billing_invoices_temp', ['company_id' => $comp_id]);
            $del_item_temp = $this->db->delete('billing_invoice_items_temp', ['billing_invoices_temp_id' => $id_temp]);
            if(!$del_temp || !$del_item_temp){
                return false;
            }
            //$this->db->update('authorized', array('status' => 'pending'), array('id' => $data['authorized_id']));
            return true;
    ...
    public function getBillingInv()
    {
        $this->db->select('billing_invoices.*, p.date as payment_date, p.reference_no as payment_ref, p.billing_id as payment_billid, p.amount as payment_amount')
            ->join('payments p', 'billing_invoices.id=p.billing_id', 'left')
            // ->where('p.billing_id', null)
            ->where("(billing_invoices.billing_status = 'pending' OR billing_invoices.billing_status = 'pending renewal')");
        $q = $this->db->get_where('billing_invoices', array('billing_invoices.company_id' => $this->session->userdata('company_id')), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return false;
    ...
    public function getBillingInvReject()
    {
        $this->db->select('billing_invoices.*, p.date as payment_date, p.reference_no as payment_ref, p.billing_id as payment_billid, p.amount as payment_amount')
            ->join('payments p', 'billing_invoices.id=p.billing_id', 'left')
            // ->where('p.billing_id', null)
            ->where('billing_invoices.payment_status', 'rejected');
        $q = $this->db->get_where('billing_invoices', array('billing_invoices.company_id' => $this->session->userdata('company_id')), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
    ...
    public function getBillingWaitingPending()
    {
        $this->db->select('billing_invoices.*, p.date as payment_date, p.reference_no as payment_ref, p.billing_id as payment_billid, p.amount as payment_amount')
            ->join('payments p', 'billing_invoices.id=p.billing_id', 'left')
            ->where("(billing_invoices.billing_status = 'waiting confirmation' OR billing_invoices.billing_status = 'waiting confirmation renewal' OR billing_invoices.billing_status = 'pending')");
        $q = $this->db->get_where('billing_invoices', array('billing_invoices.company_id' => $this->session->userdata('company_id')), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
    ...
    public function addProofPayment($data)
    {
        $bill_id = $data['billing_invoice_id'];
        ...
        if ($this->db->insert('billing_payments', $data)) {
            $billing = $this->getBillingByID($data['billing_invoice_id']);

            $data_author = ['status' => 'waiting confirmation'];
            $where_author = ['id' => $billing->authorized_id];

            $data_bill = ['billing_status' => 'waiting confirmation', 'payment_status' => 'pending'];
            $where_bill = ['id' => $bill_id];

            if ($this->db->update('billing_invoices', $data_bill, $where_bill)) {
                $this->db->update('authorized', $data_author, $where_author);
                return true;
            }
            return false;
        }
    ...
    public function addProofPaymentRenewal($data)
    {
        $bill_id = $data['billing_invoice_id'];
        $comp_id = $data['company_id'];
        $cek = $this->db->get_where('billing_payments', ['billing_invoice_id' => $bill_id, 'company_id'=>$comp_id])->num_rows();
        ...
        if ($this->db->insert('billing_payments', $data)) {
            $data_bill = ['billing_status'=>'waiting confirmation renewal', 'payment_status' => 'pending'];
            $where_bill = ['id'=>$data['billing_invoice_id']];
            if(!$this->db->update('billing_invoices', $data_bill, $where_bill)){
                return false;
            }
            return true;
    ...
    public function addInvRenewal($data_billing, $addon)
    {
        $comp_id = $this->session->userdata('company_id');
        $cek = $this->db->get_where('billing_invoices_temp', ['company_id' => $comp_id]);

        if($cek->num_rows() > 0){
            $cek_item = $this->db->get_where('billing_invoice_items_temp', ['billing_invoices_temp_id' => $cek->row()->id])->num_rows();
            if($cek_item > 0){
                $this->db->delete('billing_invoice_items_temp', ['billing_invoices_temp_id' => $cek->row()->id]);
            }
            $this->db->delete('billing_invoices_temp', ['company_id' => $comp_id]);
        }
        if ($this->db->insert('billing_invoices_temp', $data_billing)) {
            $billing_invoice_temp_id = $this->db->insert_id();
            if ($addon) {
                foreach ($addon as $k => $item) {
                    unset($addon[$k]->id);
                    unset($addon[$k]->billing_invoice_id);
                    $addon[$k]->billing_invoices_temp_id = $billing_invoice_temp_id;
                }
                $this->db->insert_batch('billing_invoice_items_temp', $addon);
            }
            return $billing_invoice_temp_id;
        }
        return false;
    }

    public function addBillingPayment($data, $id)
    {
        if ($this->db->update('billing_payments', $data, array('billing_invoice_id' => $id)) && $this->db->update('billing_invoices', array('payment_status' => 'paid'), array('id' => $id))) {
            // $this->site->updateReference('bpay');
            $this->updateAuthorized($id, $data['updated_at']);
        }
        return false;
    }
    ...
    {
        $this->db->select('payment_status as inv_pay, 
            company_name, 
            p.reference_no as payment_ref, 
            p.date as payment_date, 
            p.amount, 
            p.amount as payment_amount,
            billing_invoices.date as date, 
            billing_invoices.reference_no as reference_no, 
            billing_invoices.payment_status as payment_status, 
            billing_invoices.id as billing_id, 
            billing_invoices.id as id, 
            billing_invoices.date as inv_date, 
            billing_invoices.billing_status as billing_status, 
            billing_invoices.reference_no as inv_ref, 
            billing_invoices.company_id, 
            billing_invoices.plan_name, 
            billing_invoices.plan_id, 
            billing_invoices.created_by, 
            billing_invoices.payment_period, 
            billing_invoices.subtotal, 
            billing_invoices.price, 
            billing_invoices.total as total_inv,
            billing_invoices.total as total,
            authorized_id, due_date')
            ->join('payments p', 'p.billing_id=billing_invoices.id', 'left')
            ->where('billing_invoices.id', $id);
    ...
    public function getPaymentByBID($id)
    {
        $q = $this->db->get_where('payments', array('billing_id' => $id), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return false;
    ...
    public function addPaymentBillingInv($data)
    {
        if ($this->db->insert('payments', $data) && $this->db->update('billing_invoices', array('payment_status' => 'paid'), array('id' => $data['billing_id']))) {
            if ($data['type'] == 'received') {
                $this->updateAuthorized($data['billing_id'], $data['date']);
            }
            return true;
        }
        return false;
    ...
    public function addPaymentBillingInvRenewal($data, $billing)
    {
        if ($this->db->insert('payments', $data)) {
            $author = $this->getAuthor($billing->company_id);
    ...
    public function updateAuthorized($billing_id, $date)
    {
        $billing = $this->getBillInvByID($billing_id);
        ...
        $binv_item = $this->getBillingInvItem($billing_id);
        ...
        $data_bill = ['billing_status' => 'active', 'start_date' => $date, 'end_date' => $end];
        $where_bill = ['id' => $billing_id];

        if ($this->db->update('authorized', $data_author, $where_author) && $this->db->update('billing_invoices', $data_bill, $where_bill)) {
            /*if (strtolower($billing->plan_name)=='free') {
                $this->db->update('authorized', array('start_date'=>null, 'expired_date'=>null), array('company_id'=>$billing->company_id));
            }*/
            return true;
        }
    }

    public function confirmPaymentBilling($transaction)
    {
        if ($transaction->status_code == "200") {
            if (
                $this->db->update('payments', array('type' => 'received'), array('billing_id' => $transaction->order_id)) &&
                $this->updateAuthorized($transaction->order_id, $transaction->transaction_time)
            ) {
                return true;
            }
        }
    ...
    {
        $company_id = $this->session->userdata('company_id');
        $this->db->select('*, MAX(id) as id_bill');
        $q = $this->db->get_where('billing_invoices', array('company_id' => $company_id, 'billing_status' => 'active'));
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return false;
    ...
    public function getPendingBill()
    {
        $company_id = $this->session->userdata('company_id');
        $this->db->where('company_id', $company_id);
        $this->db->where("(billing_status = 'waiting confirmation' OR billing_status = 'waiting confirmation renewal' OR billing_status = 'pending' OR billing_status = 'rejected' )");
        $q = $this->db->get('billing_invoices');
        if ($q->num_rows() > 0) {
            return $q->row();
        }
    ...
    public function cekactiveBilling()
    {
        $q = $this->db->get_where('billing_invoices', array('billing_status' => 'active'));
        if ($q->num_rows() > 0) {
            return $q->result();
        }
        return false;
    ...
    public function updateBilling($data, $where)
    {
        if ($this->db->update('billing_invoices', $data, $where)) {
            return true;
        }
        return false;
    }

    public function getWaitingBill()
    {
        $this->db->where('billing_status', 'waiting confirmation');
        $this->db->or_where('billing_status', 'waiting confirmation renewal');
        $q = $this->db->get('billing_invoices');
        if ($q->num_rows() > 0) {
            return $q;
        }
    ...
    public function getExpiredBill()
    {
        $company_id = $this->session->userdata('company_id');
        $q = $this->db->get_where('authorized', array('company_id'=>$company_id))->row();
        $exp_date = strtotime ($q->expired_date);
        $warning_date = strtotime('-15 day', $exp_date);
        $now = strtotime(date('Y-m-d H:i:s'));

        if ($q->plan_id == 1) {
            return false;
        } else {
            if (($now >= $warning_date && $now < $exp_date) || ($now >= $exp_date)) {
                return true;
            }
            return false;
        }
    ...
    public function getPaymentReject($id = null)
    {
        if ($id) {
            $company_id = $id;
        } else {
        ...
        $q = $this->db->get_where('billing_invoices', ['payment_status' => 'rejected', 'company_id' => $company_id]);
        if ($q->num_rows() > 0) {
            return true;
        }
        return false;
    ...
    public function addBillingTemp($data, $addon=null)
    {
        $comp_id = $this->session->userdata('company_id');
        $cek = $this->db->get_where('billing_invoices_temp', ['company_id' => $comp_id]);
        ...
        if ($this->db->insert('billing_invoices_temp', $data)) {
            $billing_invoice_temp_id = $this->db->insert_id();
            if ($addon) {
                foreach ($addon as $item) {
                    $item['billing_invoices_temp_id'] = $billing_invoice_temp_id;
                    $this->db->insert('billing_invoice_items_temp', $item);
                }
            }
            return true;
        }
    ...
    public function getBillingTemp()
    {
        $comp_id = $this->session->userdata('company_id');

        $this->db->where('company_id', $comp_id);
        $q = $this->db->get('billing_invoices_temp');
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return false;
    ...
    {
        $this->db->select('users.*, groups.name as group, warehouses.name as warehouses, companies.client_id as client_id');
        $this->db->join('warehouses', 'warehouses.id = users.warehouse_id');
    ...
    public function getBillingTempByID($id)
    {
        $this->db->where('id', $id);
        $q = $this->db->get('billing_invoices_temp');
        if ($q->num_rows() > 0) {
    ...
    public function getBillingItemTemp($id)
    {
        $this->db->where('billing_invoices_temp_id', $id);
        $q = $this->db->get('billing_invoice_items_temp');
        if ($q->num_rows() > 0) {
            return $q->result();
        }
        return false;
    ...
    {   
        ...
        $this->db
            ->select('authorized.*, companies.company, sma_join.email as email')
            ->join('companies', 'authorized.company_id = companies.id', 'left')
            ->join($join, 'sma_join.company_id = authorized.company_id', 'left')
            ->where('authorized.plan_id', 2)
            ->where('authorized.status', 'activated')
            ->from("authorized");
    ...
    //---------------------------------------------------------------------------------------------------------------------------------------//
    public function findUsernameUsers($username)
    {
