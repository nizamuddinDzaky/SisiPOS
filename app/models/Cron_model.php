...
class Cron_model extends CI_Model
{
    ...
    public function run_cron()
    {
        ...
        if ($unpaidpurchases = $this->getUnpaidPuchases()) {
            ...
            foreach ($unpaidpurchases as $purchase) {
                $this->db->update('purchases', array('updated_at' => date('Y-m-d H:i:s'), 'payment_status' => 'due'), array('id' => $purchase->id));
                $up++;
            }
    ...
    {
        if ($this->db->update('sales', array('updated_at' => date('Y-m-d H:i:s'), 'payment_status' => 'due'), array('id' => $id))) {
            return true;
        }
        return false;
    }
        ...
        if ($this->Settings->reference_format == 1 || $this->Settings->reference_format == 2) {
            ...
            if ($ref = $this->getOrderRef()) {
                ...
                if ($this->Settings->reference_format == 1 && strtotime($ref->date) < strtotime($year)) {
                    $reset_ref['date'] = $year;
                    //original $this->db->update('order_ref', $reset_ref, array('ref_id' => 1));
                    $this->db->update('order_ref', $reset_ref, '');
                    return true;
    ...
    public function sent_email($details)
    {
        if ($details) {
            $table_html = '';
