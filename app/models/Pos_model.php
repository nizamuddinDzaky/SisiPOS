...
class Pos_model extends CI_Model
{
    ...
    public function getOpenRegisters()
    {
        ...
        $q = $this->db->get_where('pos_register', array('status' => 'open','users.company_id'=>$user_id = $this->session->userdata('company_id')));
        if ($q->num_rows() > 0) {
    ...
    public function getWHProduct($code, $warehouse_id)
    {
        ...
        $q = $this->db->get_where("products", array('products.code' => $code,'warehouses_products.warehouse_id'=>$warehouse_id));
        if ($q->num_rows() > 0) {
            return $q->row();
        }
    ...
    public function addSale($data = array(), $items = array(), $payments = array(), $sid = null)
    {
        ...
        if ($this->db->insert('sales', $data)) {
            ...
            //$ref = SmaOrderRef::updateRef('pos');
            $this->site->updateReference('pos');
            foreach ($items as $item) {
                $item['sale_id'] = $sale_id;
                $this->db->insert('sale_items', $item);
                $sale_item_id = $this->db->insert_id();
                if ($data['sale_status'] == 'completed' && $this->site->getProductByID($item['product_id']) && !$item['flag']) {
                ...
                foreach ($payments as $payment) {
                    if (!empty($payment) && isset($payment['amount']) && $payment['amount'] != 0) {
                        ...
                        if ($payment['paid_by'] == 'ppp') {
                            ...
                            $count_used_points=$count_used_points+$payment['using_point'];
                            unset($payment['cc_cvv2']);
                            unset($payment['using_point']);
                            $this->db->insert('payments', $payment);
                            //$ref = SmaOrderRef::updateRef('pay');
                            $this->site->updateReference('pay');
                            $paid += $payment['amount'];
            ...
            $this->sma->update_award_points($data['grand_total'], $data['customer_id'], $data['created_by'], null, $count_used_points);
        ...
        if (isset($payment['sale_id']) && isset($payment['paid_by']) && isset($payment['amount'])) {
            ...
            if ($payment['paid_by'] == 'ppp') {
                ...
                if ($payment['paid_by'] == 'gift_card') {
                    ...
                    $this->db->update('companies', array('updated_at' => date('Y-m-d H:i:s'), 'deposit_amount' => ($customer->deposit_amount-$payment['amount'])), array('id' => $customer_id));
                }
                unset($payment['cc_cvv2']);
                $this->db->insert('payments', $payment);
                $paid += $payment['amount'];
    ...
    public function getGross($pid, $wid, $qty)
    {
        $condition="(CASE 
            WHEN operation='>'  THEN greater 
            WHEN operation='<'  THEN less 
            WHEN operation='>=' THEN e_greater 
            WHEN operation='<=' THEN e_less 
            ELSE equal 
        END)";
        $temp_table="(SELECT g.*, p.name as pname, w.name as wname, ".$qty.">g.quantity as greater, ".$qty."<g.quantity as less, ".$qty.">=g.quantity as e_greater, ".$qty."<=g.quantity as e_less, ".$qty."=g.quantity as equal 
            FROM ".$this->db->dbprefix('gross')." as g 
            LEFT JOIN ".$this->db->dbprefix('products')." as p on g.product_id=p.id 
            LEFT JOIN ".$this->db->dbprefix('warehouses')." as w on g.warehouse_id=w.id 
            WHERE g.product_id=".$pid." 
            AND g.warehouse_id=".$wid.") as temp_table";
        $q=$this->db->query("SELECT id, pname, wname, quantity, operation, price, ".$condition." as result FROM ".$temp_table."
    ...
    public function getMultiDisc($pid, $wid, $qty)
    {
        $condition="(CASE 
            WHEN operation='>'  THEN greater 
            WHEN operation='<'  THEN less 
            ...
            ELSE equal 
        END)";
        $temp_table="(SELECT md.*, p.name as pname, w.name as wname, ".$qty.">md.quantity as greater, ".$qty."<md.quantity as less, ".$qty.">=md.quantity as e_greater, ".$qty."<=md.quantity as e_less, ".$qty."=md.quantity as equal 
            FROM ".$this->db->dbprefix('multiple_discount')." as md 
            LEFT JOIN ".$this->db->dbprefix('products')." as p on md.product_id=p.id 
            LEFT JOIN ".$this->db->dbprefix('warehouses')." as w on md.warehouse_id=w.id 
            WHERE md.product_id=".$pid." 
            AND md.warehouse_id=".$wid.") as my_table";
        $q=$this->db->query("SELECT id, pname, wname, quantity, operation, discount, sub_discount, ".$condition." as result FROM ".$temp_table."
                WHERE (is_deleted=0 OR is_deleted is NULL)
                AND start_date < NOW() AND (end_date is NULL or end_date > NOW())
                AND ".$condition."=1
                ORDER BY quantity DESC LIMIT 1");
    ...
    public function getProductBonus($pid, $wid, $qty)
    {
        $this->db->select('p.name as pname, p.code as pcode, w.name, pb.name as bonus_name, pb.code as bonus_code, bonus.quantity as qty, multiply')
                ->join('products as p', 'bonus.product_id=p.id', 'left')
                ->join('warehouses as w', 'bonus.warehouse_id=w.id', 'left')
                ->join('products as pb', 'bonus.product_bonus=pb.id', 'left')
                ->where('warehouse_id', $wid)
                ->where('product_id', $pid)
                ->where('bonus.quantity<=', $qty)
                ->group_start()->where('bonus.is_deleted', null)->or_where('bonus.is_deleted', 0)->group_end()
//                ->where('start_date <', 'NOW()',false)->group_start()->where('end_date',null)->or_where('end_date >', 'NOW()', false)->group_end()
                ->order_by('bonus.quantity', 'desc');
        $q=$this->db->get('bonus');
        if ($q->num_rows() > 0) {
            return $q->row();
        }
