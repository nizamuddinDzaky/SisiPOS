...
class Sync_model extends CI_Model
{
    ...
    public function importBillers()
    {
        ...
        if ($billers) {
            foreach ($billers as $biller) {
                ...
                $biller->created_at = date('Y-m-d H:i:s');
            ...
            if (isset($ids)) {
                krsort($ids);
                foreach ($ids as $id) {
                    $this->db->update('sales', array('updated_at' => date('Y-m-d H:i:s'), 'biller_id' => $id['new']), array('biller_id' => $id['old']));
    ...
    public function importCustomers()
    {
        ...
        if ($customers) {
            ...
            if (isset($ids)) {
                krsort($ids);
                foreach ($ids as $id) {
                    $this->db->update('sales', array('updated_at' => date('Y-m-d H:i:s'), 'customer_id' => $id['new']), array('customer_id' => $id['old']));
    ...
    public function importSuppliers()
    {
        ...
        if ($suppliers) {
            ...
            if (isset($ids)) {
                krsort($ids);
                foreach ($ids as $id) {
                    $this->db->update('purchases', array('updated_at' => date('Y-m-d H:i:s'), 'supplier_id' => $id['new']), array('supplier_id' => $id['old']));
                }
            }
            return true;
        }
    ...
    {
        $this->db->query("UPDATE " . $this->db->dbprefix('purchases') . " SET paid=grand_total, status='received', payment_status='paid', updated_at=NOW()");
        return true;
    }
