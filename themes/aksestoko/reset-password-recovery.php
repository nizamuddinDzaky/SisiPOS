        ...
        <div class="col-lg-6 align-self-center">
          <div class="card">
            ...
            <div class="card-body">
              ...
              <h6 class="card-subtitle pb-4 text-muted text-left">Reset kata sandi menggunakan kode pemulihan yang didapatkan saat pendaftaran. <span class="text-info">(Contoh: c0d40f1e-ee2e-4255-b086-d82345bd)</span></h6>
              ...
              <div class="card-text">
                ...
                <div class="form-group pt-3">
                  <input type="text" autofocus class="form-control form-control-custom text-sans-serif text-center" id="recovery_code" name="recovery_code" placeholder="Masukkan kode pemulihan">
                </div>
                <small class="text-danger">Valid sampai dengan <span><?=date('d F Y H:i', strtotime($reset_password->valid_until))?></span></small>
                <div class="form-group">
                  <input type="hidden" value="<?=$this->session->userdata('id_forget_password')?>" id="id_fp">
                  <input type="text" required readonly class="form-control form-control-custom text-sans-serif text-center bg-white d-none" id="otp" name="otp" maxlength="5">
                </div>
                
                <button class="btn btn-l btn-block btn-primary mb-1 mt-3" onclick="checkRecoveryCode()">Cek</button>
...
<!-- END MODAL -->
  ...
  <script>
    function checkRecoveryCode(){
      if($("#recovery_code").val().length != 32) {
        return alert("Panjang Kode Pemulihan adalah 32 digit")
      }
      let data = {
      ...
      $.ajax({
          ...
          dataType: "json",
          data,
          url: '<?=base_url(aksestoko_route('aksestoko/auth/check_recovery_code'))?>',
          success: function(response){
            $("#otp").val(response.otp_code);
            checkOtp();
          }, 
