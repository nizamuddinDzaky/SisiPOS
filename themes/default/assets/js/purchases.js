$(document).ready(function () {
    ...
    $('#no_si_spj').change(function (e) {
        localStorage.setItem('no_si_spj', $(this).val());
    });
    if (no_si_spj = localStorage.getItem('no_si_spj')) {
        $('#no_si_spj').val(no_si_spj);
    }
    $('#no_si_do').change(function (e) {
        localStorage.setItem('no_si_do', $(this).val());
    });
    if (no_si_do = localStorage.getItem('no_si_do')) {
        $('#no_si_do').val(no_si_do);
    }
    $('#no_si_so').change(function (e) {
        localStorage.setItem('no_si_so', $(this).val());
    });
    if (no_si_so = localStorage.getItem('no_si_so')) {
        $('#no_si_so').val(no_si_so);
    }
    $('#no_si_billing').change(function (e) {
        localStorage.setItem('no_si_billing', $(this).val());
    });
    if (no_si_billing = localStorage.getItem('no_si_billing')) {
        $('#no_si_billing').val(no_si_billing);
    }
    $('#delivery_date').change(function (e) {
        localStorage.setItem('delivery_date', $(this).val());
    });
    if (delivery_date = localStorage.getItem('delivery_date')) {
        $('#delivery_date').val(delivery_date);
    }
    $('#cf1').change(function (e) {
        localStorage.setItem('cf1', $(this).val());
    });
    if (delivery_date = localStorage.getItem('cf1')) {
        $('#cf1').val(delivery_date);
    }
    ...
    $('#reset').click(function (e) {
        bootbox.confirm(lang.r_u_sure, function (result) {
            if (result) {
                ...
                if (localStorage.getItem('cf1')) {
                    localStorage.removeItem('cf1');
                }
                ...
                if (localStorage.getItem('poorder')) {
                    localStorage.removeItem('poorder');
                }
    ...
    $(document).on('click', '.podel', function () {
        ...
        if (poitems.hasOwnProperty(item_id)) { } else {
            localStorage.setItem('poitems', JSON.stringify(poitems));
            item = JSON.parse(localStorage.getItem('poitems'));
            if ($.isEmptyObject(item) && localStorage.getItem('posupplier')) {
                //                localStorage.removeItem('posupplier');
    ...
    $(document).on("focus", '.rquantity', function () {
        ...
        if (poitems[item_id].row.received > poitems[item_id].row.qty) {
            $(this).val(old_row_qty);
            bootbox.alert(lang.unexpected_value);
            return;
        }
    ...
    $(document).on("focus", '.received', function () {
        ...
        if (!is_numeric(new_received) || new_received < 0) {
    ...
    if (localStorage.getItem('poitems')) {
        ...
        $.each(sortedItems, function () {
            ...
            $(document).on('click', 'i', function () {
                ...
                var a=JSON.parse(localStorage.getItem('poitems_temp'));
                if(a.data){
                    for(var i=0;i<Object.keys(a.data).length;i++){
                        ...
                        if(obj==null){
                            ...
                            if(str_itemId==obj.trx_id){
                                delete a.data[i];
                                localStorage.setItem('poitems_temp', JSON.stringify(a));
                            }
                        }
                    }
                }
...
function check_item_local_storage(item) {
    poitems = JSON.parse(localStorage.getItem('poitems'));
    let flag = true;
    if (poitems != null && Object.keys(poitems).length > 0) {
        jQuery.each(poitems, function (i, val) {
...
function add_purchase_item(item) {
    ...
    /*    if (count == 1) {
        ...
        if(localStorage.getItem('poitems_temp')){
            poitems_temp=JSON.parse(localStorage.getItem('poitems_temp'));
        }
    
        var flag=0;
        var count=Object.keys(poitems_temp.data).length;
        if(count==0){
            poitems_temp.data.push({ 
                "trx_id"        : item.id,
            ...
            for(var i=0;i<count;i++){
                var obj=poitems_temp.data[i];
                if(obj==null){
                    continue;
                }
                else{
                    if(obj.product_id==item.item_id){
                        poitems[obj.trx_id].row.qty = parseFloat(poitems[obj.trx_id].row.qty) + 1;
                        delete poitems[item_id];
                        flag=0;
                        break;
                    }
        ...
        localStorage.setItem('poitems', JSON.stringify(poitems));
        localStorage.setItem('poitems_temp', JSON.stringify(poitems_temp));
        loadItems();
        return true;*/
    if (!localStorage.getItem('posupplier')) {
        localStorage.setItem('posupplier', $('#posupplier').val());
        $('#supplier_id').val($('#posupplier').val());
    ...
    if (!check_item_local_storage(item)) {
        bootbox.alert(lang.item_purchase_duplicated, function () { });
    } else {
        if (count == 1) {
            poitems = {};
            if ($('#posupplier').val()) {
