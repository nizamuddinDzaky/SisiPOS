...
/*            $(document).on('click', 'i', function () {
                ...
                var a=JSON.parse(localStorage.getItem('quitems_temp'));
                if(a.data){
                    for(var i=0;i<Object.keys(a.data).length;i++){
                        ...
                        if(obj==null){
                            ...
                            if(str_itemId==obj.trx_id){
                                delete a.data[i];
                                localStorage.setItem('quitems_temp', JSON.stringify(a));
                            }
                        }
                    }
                }
...
function check_item_local_storage(item) {
    quitems = JSON.parse(localStorage.getItem('quitems'));
    let flag = true;
    if (quitems != null && Object.keys(quitems).length > 0) {
        jQuery.each(quitems, function (i, val) {
...
/*    if (count == 1) {
    ...
    if(localStorage.getItem('quitems_temp')){
        quitems_temp=JSON.parse(localStorage.getItem('quitems_temp'));
    }

    var flag=0;
    var count=Object.keys(quitems_temp.data).length;
    if(count==0){
        quitems_temp.data.push({ 
            "trx_id"        : item.id,
        ...
        for(var i=0;i<count;i++){
            var obj=quitems_temp.data[i];
            if(obj==null){
                continue;
            }
            else{
                if(obj.product_id==item.item_id){
                    quitems[obj.trx_id].row.qty = parseFloat(quitems[obj.trx_id].row.qty) + 1;
                    delete quitems[item_id];
                    flag=0;
                    break;
                }
    ...
    localStorage.setItem('quitems', JSON.stringify(quitems));
    localStorage.setItem('quitems_temp', JSON.stringify(quitems_temp));
    loadItems();
    return true;*/
    ...
    if (!check_item_local_storage(item)) {
        bootbox.alert(lang.item_quotation_duplicated, function () { });
    } else {
        if (count == 1) {
            quitems = {};
            if ($('#quwarehouse').val() && $('#qucustomer').val()) {
