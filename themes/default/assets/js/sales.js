$(document).ready(function (e) {
	...
	if (localStorage.getItem('slcustomer')) {
		$.ajax({
			type: "get",
			async: false,
			url: site.base_url + "sales/creditlimit/" + localStorage.getItem('slcustomer'),
			dataType: "json",
	...
	$customer.change(function (e) {
		...
		$.ajax({
			type: "get",
			async: false,
			url: site.base_url + "sales/creditlimit/" + $(this).val(),
			dataType: "json",
			success: function (data) {
				$("#slkredit").val(data);
			}
				...
				results: function (data, page) {
					if (data.results != null) {
						...
						for (var i = 0; i < data.results.length; i++){
						    CostumResult.push({"id" : data.results[i].id, text: data.results[i].text });
						}
						return {results:CostumResult};
	...
	if(pay_term != 'add'){ // untuk edit_sales_booking
		let cek_p = '';
		$.each(JSON.parse(arr_pay_term), function (i, v) {
			if(pay_term == v){
				cek_p = 1;
			}
		});
		if(cek_p == '') {
			$('#s2id_slpayment_term').hide();
			$('#slpayment_term').removeAttr('name');			
		}
		else{
			$('#slpayment_term_m').hide();
			$('#slpayment_term_m').removeAttr('name');
		}
	}
	else{ // untuk add_sales_booking
		$('#slpayment_term_m').hide();
	}
	$('#slpayment_term_check').click(function () {
		var payterm = $(this).closest('.input-group').find("input[name='payment_term' id='slpayment_term']");
		if($('#slpayment_term_m').attr('name')){
			$('#slpayment_term').attr('name','payment_term');
			$('#slpayment_term').addClass("form-control input-tip");
			$('#s2id_slpayment_term').show();
			$('#slpayment_term_m').removeAttr('name');
			$('#slpayment_term_m').hide();
			$('#slpayment_term_m').addClass("form-control input-tip");
			$('#slpayment_term_m').removeAttr('name');
			$("#slpayment_term option:first").attr('selected','selected');
			...
			$('#slpayment_term').addClass("form-control input-tip");
			$('#s2id_slpayment_term').hide();
			$('#slpayment_term_m').attr('name','payment_term');
			$('#slpayment_term_m').show();
			$('#slpayment_term_m').addClass("form-control input-tip");
		}
	});
	$('#slpayment_status').change(function (e) {
		...
		if (ps == 'partial' || ps == 'paid') {
			if (ps == 'paid') {
				...
				$('#display_input_payment_term').hide();
				$('#slpayment_term').val(0);
			}
			else{
				$('#display_input_payment_term').show();
			}
			$('#payments').slideDown();
			$('#pcc_no_1').focus();
		} else {
	...
	$('#add_sale, #edit_sale, #add_booking_sale, #edit_booking_sale').attr('disabled', true);
	...
	$('#reset').click(function (e) {
		bootbox.confirm(lang.r_u_sure, function (result) {
			if (result) {
				...
				if (localStorage.getItem('slcharge')) {
					localStorage.removeItem('slcharge');
				}
				if (localStorage.getItem('slreason')) {
					localStorage.removeItem('slreason');
				}
	...
	$('input[name="price_type"]').on('ifClicked', function(e){
	  	localStorage.setItem('slprice_type', $(this).val());
	});
	if (slprice_type = localStorage.getItem('slprice_type')) {
		$('input[name="price_type"]').filter('[value="'+slprice_type+'"]').iCheck('check');
	}
	$('#slreason').redactor('destroy');
	$('#slreason').redactor({
		buttons: ['formatting', '|', 'alignleft', 'aligncenter', 'alignright', 'justify', '|', 'bold', 'italic', 'underline', '|', 'unorderedlist', 'orderedlist', '|', 'link', '|', 'html'],
		formattingTags: ['p', 'pre', 'h3', 'h4'],
		minHeight: 100,
		changeCallback: function (e) {
			var v = this.get();
			localStorage.setItem('slreason', v);
		}
	});
	if (slreason = localStorage.getItem('slreason')) {
		$('#slreason').redactor('set', slreason);
	}
	...
	var old_charge;
	$('#slcharge').focus(function () {
		old_charge = $(this).val();
	}).change(function () {
		var new_charge = $(this).val() ? $(this).val() : '0';
		if (is_valid_discount(new_charge)) {
			localStorage.removeItem('slcharge');
			localStorage.setItem('slcharge', new_charge);
			loadItems();
			return;
		} else {
			$(this).val(old_charge);
	...
	$(document).on('click', '.sldel', function () {
		...
		if(slitems[item_id].client_id == 'atl'){
			bootbox.alert('AksesToko Product can not be deleted');
			return
		}
	...
	$(document).on('click', '#editItem', function () {
		...
		if (price_type == 'cash') {
			...
			slitems[item_id].row.real_unit_price_credit = price;
		}
	...
	* Sell Gift Card modal
	...
	$(document).on('click', '#sellGiftCard', function (e) {
		if (count == 1) {
			...
			if ($('#slwarehouse').val() && $('#slcustomer').val()) {
				...
				$('input[name="price_type"],[type="radio"]').iCheck('disable');
			} else {
	...
	// Mendapatkan credit limit  sebelum dikurangi grand total saat edit sale
	var url = window.location.href.split('/');
	if (url[url.length - 2] == 'edit') {
		gtotal = parseInt($('#gtotal').text().split('.').join(""));
		credit = parseInt($("#slkredit").val().split('.').join(""));

		var current = credit + gtotal;
		var numberFormat2 = new Intl.NumberFormat('de-DE');
		var number = numberFormat2.format(current)
		$("#slkredit").val(number);
...
function loadItems() {
	...
	if (localStorage.getItem('slitems')) {
		...
		if (localStorage.getItem('slprice_type')) {
			var price_type = localStorage.getItem('slprice_type');
		} else{
			var price_type = $('input[name="price_type"]:checked').val();
		}
		...
		$('#add_sale, #edit_sale, #add_booking_sale, #edit_booking_sale').attr('disabled', false);
		/* untuk notifikasi booking */
		let kurang = '';
		$('#bookingnotif').html('');
		$.each(sortedItems, function () {
			...
			var product_id = item.row.id,
				...
				item_qty = parseFloat(item.row.qty),
			...
			var product_unit = item.row.unit,
				base_quantity = parseFloat(item.row.base_quantity);
			var consignment = item.row.cons;
			let item_qty_booking = parseFloat(item.row.qty_book_wh);
			let item_qty_wh = parseFloat(item.row.qty_wh ? item.row.qty_wh : item.row.quantity);
			...
			if(price_type == 'cash'){
				...
				var unit_price = parseFloat(item.row.real_unit_price_credit);
				var item_price = item.row.price_credit;
			}
			...
			class_hide = '';
			hide_icon = '';
			if (this.client_id == 'aksestoko') {
				disabled = 'readonly';
				class_hide = 'hide';
			} else if (this.client_id == 'atl'){
				disabled = 'readonly';
			}

			var row_no = (new Date).getTime();
			var newTr = $('<tr id="row_' + row_no + '" class="row_' + item_id + '" data-item-id="' + item_id + '"></tr>');
			tr_html = '<td><input name="product_id[]" type="hidden" class="rid" value="' + product_id + '"><input name="product_type[]" type="hidden" class="rtype" value="' + item_type + '"><input name="product_code[]" type="hidden" class="rcode" value="' + item_code + '"><input name="product_name[]" type="hidden" class="rname" value="' + item_name + '"><input name="product_option[]" type="hidden" class="roption" value="' + item_option + '"><span class="sname" id="name_' + row_no + '">' + item_code + ' - ' + item_name + (sel_opt != '' ? ' (' + sel_opt + ')' : '') + '</span> <i class="pull-right fa fa-edit tip pointer edit ' + class_hide + '" id="' + row_no + '" data-item="' + item_id + '" title="Edit" style="cursor:pointer;"></i><input class="form-control input-sm" name="consignment[]" type="hidden" id="consignment_' + row_no + '" value="' + consignment + '"></td>';
			if (site.settings.product_serial == 1) {
				tr_html += '<td class="text-right"><input class="form-control input-sm rserial" name="serial[]" type="text" id="serial_' + row_no + '" value="' + item_serial + '" ' + disabled + '></td>';
			}
			...
			tr_html += '<td class="text-center"><i class="fa fa-times tip pointer sldel ' + class_hide + '" id="' + row_no + '" title="Remove" style="cursor:pointer;"></i></td>';

			//			$(document).on('click', 'i', function () {
			//				var str_itemId=$(this).closest('tr').attr('data-item-id');
			//    			var a=JSON.parse(localStorage.getItem('slitems_temp'));
			//    			if(a.data){
			...
			//				 				delete a.data[i];
			//				 				localStorage.setItem('slitems_temp', JSON.stringify(a));
			//				 			}
			//			 			}
			//			 		}
			//    			}
			...
			if (item_type == 'standard' && item.options !== false) {
				$.each(item.options, function () {
					if (this.id == item_option && (base_quantity > item_qty_wh)) {
			...
			} else if (item_type == 'standard' && base_quantity > item_qty_wh) {
				kurang += 'Quantity ' + item.row.name + '<br> di Warehouse kurang <br>';
				$('#bookingnotif').html(kurang);
				$('#row_' + row_no).addClass('danger');
				if (site.settings.overselling != 1) {
					$('#add_sale, #edit_sale').attr('disabled', true);
				}
				...
				if (combo_items === false) {
					...
					$.each(combo_items, function () {
						if (parseFloat(item_qty_wh) < (parseFloat(this.qty) * base_quantity) && this.type == 'standard') {
							$('#row_' + row_no).addClass('danger');
							if (site.settings.overselling != 1) {
								$('#add_sale, #edit_sale').attr('disabled', true);
							}
		...
		if (slcharge = localStorage.getItem('slcharge')) {
			charge = formatDecimal(slcharge);
			//total_discount += parseFloat(order_discount);
		}
		...
		var gtotal = parseFloat(((total + invoice_tax) - order_discount) + shipping + charge);
		...
		$('#charge').text(formatMoney(charge));
		$('#tship').text(formatMoney(shipping));
		$('#gtotal').text(formatMoney(gtotal));
		...
		if (count > 1 && localStorage.slsale_status != 'pending') {
			$('#slcustomer').select2("readonly", true);
			$('#slwarehouse').select2("readonly", true);
			$('input[name="price_type"],[type="radio"]').iCheck('disable');
		}
...
slitems = {};*/
function check_item_local_storage(item) {
	slItems = JSON.parse(localStorage.getItem('slitems'));
	let flag = true;
	if (slItems != null && Object.keys(slItems).length > 0) {
		jQuery.each(slItems, function (i, val) {
			if (val.item_id == item.row.id) {
				flag = false;
			}
		});
	}
...
function add_invoice_item(item) {
	/*    if (count == 1) {
		...
		if(localStorage.getItem('slitems_temp')){
			slitems_temp=JSON.parse(localStorage.getItem('slitems_temp'));
			var total=0;var contain=0;
			for(var i=0;i<Object.keys(slitems_temp.data).length;i++){
				if(slitems_temp.data[i]==null){
					contain++;
				}total++;
			}
			if(total==contain){
		...
		var flag=0;
		var count=Object.keys(slitems_temp.data).length;
		if(count==0){
			...
			for(var i=0;i<count;i++){
				var obj=slitems_temp.data[i];
				if(obj==null){
					continue;
				}else{
					if(obj.product_id==item.item_id){
						slitems[obj.trx_id].row.qty = parseFloat(slitems[obj.trx_id].row.qty) + 1;
						delete slitems[item_id];
						flag=0;
						break;
					}else{
	...
	if (!check_item_local_storage(item)) {
		bootbox.alert(lang.item_sale_duplicated, function () {});
	} else {
		if (count == 1) {
			slitems = {};
			if ($('#slwarehouse').val() && $('#slcustomer').val()) {
...
//});
/* ------------------------------
	 * Submit form check imit credit & modal warning limit credit 
	 ------------------------------- */
...
$('#edit_sale').click(function () {
	...
	if (gtotal > credit) {
		...
		$("#edit_sale").attr("disabled", "disabled");
...
$('#add_booking_sale').click(function () {
	var dangerRow = $('#slTable tr.danger');
	var lengthDangerRow = $('#slTable tr.danger').length;
	if ($("#slkredit").val()) {
		gtotal = parseInt($('#gtotal').text().split('.').join(""));
		credit = parseInt($("#slkredit").val().split('.').join(""));
		...
		if (gtotal > credit) {
			...
			if (lengthDangerRow > 0) {
				...
				document.getElementById('formfield').submit();
				$("#add_booking_sale").attr("disabled", "disabled");
			}
		}
	}
...
$('#edit_booking_sale').click(function () {
	/* when the button in the form, display the entered values in the modal */
	$(window).unbind('beforeunload');
	if ($("#slkredit").val()) {
		var dangerRow = $('#slTable tr.danger');
...
$('#submit-sale-booking').click(function () {
	var x = document.getElementById('bookingnotif');
	if (x.style.display === 'none') {
		document.getElementById('formfield').submit();
	} else {
		$('#creditlimit').show();
		var box = document.getElementsByClassName('modal-dialog');
		box.style.width = '40%';
		$('#bookingnotif').hide();
	}
});
