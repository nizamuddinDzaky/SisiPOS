$(document).ready(function(){
    ...
    $(document).on('click', 'tr:has(td .oprice)', function(e) {
        var row = $(this).closest('tr');
        var _idrow = row.attr('data-item-id');
        $.each(dt_gross,function(){
            if(this.row_id==_idrow){
                $('#gprice').text('('+formatDecimal(this.quantity)+this.operation+')'+formatMoney(this.price));
            }
        });
    });
});
...
/* ----------------------
	 ...
	 * ---------------------- */
	 $("#ppshipping").click(function(e) {
	 	e.preventDefault();
	 	var spval = $('#posshipping').val() ? $('#posshipping').val() : '0';
	 	$('#order_shipping_input').val(spval);
	 	$('#spModal').modal();
	 });
	 $('#spModal').on('shown.bs.modal', function() {
	 	$(this).find('#order_shipping_input').select().focus();
	 	$('#order_shipping_input').bind('keypress', function(e) {
	 		if (e.keyCode == 13) {
	 			e.preventDefault();
	 			var ds = $('#order_shipping_input').val();
	 			if (is_valid_discount(ds)) {
	 				$('#posshipping').val(ds);
	 				localStorage.removeItem('posshipping');
	 				localStorage.setItem('posshipping', ds);
	 				loadItems();
	 			} else {
	 				bootbox.alert(lang.unexpected_value);
	 			}
	 ...
	 $(document).on('click', '#updateOrderShipping', function() {
             var ds;
             if($('#spCost').val()=='entry'){
                 ds = $('#order_shipping_input').val() ? $('#order_shipping_input').val() : '0'; 
             }else{
                 ds = $('#shipping').val() ? $('#shipping').val() : '0';
             }
             if (is_valid_discount(ds)) {
                 $('#posshipping').val(ds);
                 localStorage.removeItem('posshipping');
...
$('#posdiscount').focus(function () {
	 		...
	 		boxd.on("shown.bs.modal", function() {
	 			$( "#pos_pin" ).focus().keypress(function(e) {
	 				if (e.keyCode == 13) {
                        ...
                        var itembonus_id;
                        if(positems[item_id].hasOwnProperty('pr_bonus')){
                            itembonus_id=positems[item_id].pr_bonus;
                        }
	 		delete positems[item_id];
                        delete positems[itembonus_id];
	 		row.remove();
	 		if(positems.hasOwnProperty(item_id)) { } else {
	 			localStorage.setItem('positems', JSON.stringify(positems));
	 			loadItems();
	...
	$(document).on('click', '#editItem', function () {
		...
		positems[item_id].row.real_unit_price = price,
                ...
                positems[item_id].row.base_unit_price = price,
...
function loadItems() {
	if (localStorage.getItem('positems')) {
		...
		positems = JSON.parse(localStorage.getItem('positems'));
                dt_gross = JSON.parse(localStorage.getItem('grossitems'));
		...
		$.each(sortedItems, function () {
			...
			var product_id = item.row.id, item_type = item.row.type, combo_items = item.combo_items, item_price = item.row.price, old_price = item.row.price, item_qty = item.row.qty, item_aqty = item.row.quantity, item_tax_method = item.row.tax_method, item_ds = item.row.discount, item_discount = 0, item_option = item.row.option, item_code = item.row.code, item_serial = item.row.serial, item_name = item.row.name.replace(/"/g, "&#034;").replace(/'/g, "&#039;");
			...
			if (pos_settings.item_order == 1 && category != item.row.category_id) {
				...
				print_cate = false;
                        ...
                        item_price=pricing(item_qty, product_id, item.row.price, item.id, item_id);
                        unit_price=item_price;
			...
			tr_html += '<td class="text-right">';
                        tr_html += '<input class="form-control input-sm" name="consignment[]" type="hidden" id="consignment_' + row_no + '" value="'+consignment+'">';
                        if(old_price!=item_price){
                            tr_html+='<span class="text-right sprice oprice" id="oprice_' + row_no + '" style="font-size:12px;color:grey;">' + formatMoney(parseFloat(old_price) + parseFloat(pr_tax_val)) + '</span><br>';
                        }
...
//			console.log(localStorage.getItem('positems_temp'));
//			$(document).on('click', 'i', function () {
//				var str_itemId=$(this).closest('tr').attr('data-item-id');
//    			var a=JSON.parse(localStorage.getItem('positems_temp'));
//    			if(a.data){
...
//				 				delete a.data[i];
//				 				localStorage.setItem('positems_temp', JSON.stringify(a));
//				 			}
//			 			}
//			 		}
//    			}
//			});
			...
			an++;
				...
				$('#row_' + row_no).addClass('danger');
                                var itembonus;
                                if(positems[item_id].hasOwnProperty('pr_bonus')){
                                    itembonus=positems[item_id].pr_bonus;
                                }
                                delete positems[itembonus];
                                delete positems[item_id].pr_bonus;
			} else if(item_type == 'consignment' && base_quantity > item_aqty) {
				$('#row_' + row_no).addClass('danger');
                                var itembonus;
                                if(positems[item_id].hasOwnProperty('pr_bonus')){
                                    itembonus=positems[item_id].pr_bonus;
		...
		var gtotal = parseFloat(((total + invoice_tax) - order_discount + formatDecimal(shipping)));
		$('#total').text(formatMoney(total + invoice_tax));
		...
		if (site.settings.tax2 != 0) {
			$('#ttax2').text(formatMoney(invoice_tax));
                ...
                $('#tsp').text(formatMoney(shipping));
		$('#gtotal').text(formatMoney(gtotal));
                
                if(customer_point=calcPoint(gtotal)){
                    $('#point').text(customer_point.own_point);
                }
		if(java_applet == 1) {
...
function pricing(qty, pid, price, id_parent, id_row){
	new_price = price;
    $.ajax({
        type: "get", async:false,
        url: site.base_url+"pos/pricing/",
        dataType: "json",
        data: {
            quantity: qty,
        ...
        success: function (data) {
            if(data.gross){
                dt_gross[id_row]=data.gross;
                dt_gross[id_row].row_id=id_row;
            }
            new_price= data.gross ? data.gross.price : price;

            if(data.multidiscount){
                if((data.multidiscount.discount).indexOf('%')){
                    disc=(data.multidiscount.discount).split('%');
                    ods=formatDecimal(parseFloat(((new_price) * parseFloat($.trim(disc[0]))) / 100), 4);
                }else
                    ods=data.multidiscount.discount;
                new_price=new_price-ods;

                if((data.multidiscount.sub_discount).indexOf('%')){
                    sub_disc=(data.multidiscount.sub_discount).split('%');
                    sub_ods=formatDecimal(parseFloat(((new_price) * parseFloat($.trim(sub_disc[0]))) / 100), 4);
                }else
                    sub_ods=data.multidiscount.sub_discount;
                new_price=new_price-sub_ods;
            }

            if(data.bonus){
                $.each(positems,function(){
                    qty_pr_bonus=qty/data.bonus.qty;
                    if(Math.floor(qty_pr_bonus)!=qty_pr_bonus && $.isNumeric(qty_pr_bonus) && data.bonus.multiply){
                        qty_pr_bonus=parseInt(qty_pr_bonus);
                    }else{
                        qty_pr_bonus=1;
                    }

                    if((this.id==id_parent) && (!this.pr_bonus)){
//                        add_bonus(data.bonus.bonus_code, add_invoice_item, id_parent);
                        add_bonus(data.bonus, qty_pr_bonus, id_parent);
                    }else if(this.id==id_parent && this.pr_bonus && data.bonus.multiply){
                        positems[this.pr_bonus].row.qty=qty_pr_bonus;
                    }else
                        return false;
                });
            }else if(!data.bonus){
                $.each(positems,function(){
                    if((this.pr_parent) && (this.pr_parent==id_parent)){
                        delete positems[this.id];
                        delete positems[id_parent].pr_bonus;
                    }
                });
            }
        }
...
function add_bonus(data_bonus,qty,parent){
    $.ajax({
        type: "get",
        url: site.base_url+"sales/suggestions",
        dataType: "json",
        data: {
            term: data_bonus.bonus_code,
            warehouse_id: $("#poswarehouse").val()
        },
        success: function (data) {
            var row_bonus;
            $.each(data,function(){
                if(this.row.code==data_bonus.bonus_code){
                    row_bonus=this;
                }
            });
            $.each(positems,function(){
                if(this.id==parent)
                    this.pr_bonus=row_bonus.id;
            });
            row_bonus.row.qty=row_bonus.row.qty*qty;
            row_bonus.pr_parent=parent;
            row_bonus.row.price=0;
//            callback(data[0]);
            if(!$('.row_' + parent).hasClass('danger')){
                add_invoice_item(row_bonus);
            }else{
                delete positems[parent].pr_bonus;
            }
        }
    });
}

function calcPoint(grand_total=null,point=null){
    $.ajax({
        type: "get",async:false,
        url: site.base_url+"pos/calculate_point",
        dataType: "json",
        data: {
            total: grand_total,
            essence: point
...
/*    if (count == 1) {
    ...
    if(localStorage.getItem('positems_temp')){
        positems_temp=JSON.parse(localStorage.getItem('positems_temp'));
        var total=0;var contain=0;
        for(var i=0;i<Object.keys(positems_temp.data).length;i++){
            if(positems_temp.data[i]==null){
                contain++;
            }total++;
        }
        if(total==contain){
            delete positems_temp;
            positems_temp = {
                "data":[]
            };
        }
    ...
    var flag=0;
    var count=Object.keys(positems_temp.data).length;
    if(count==0){
        ...
        for(var i=0;i<count;i++){
            var obj=positems_temp.data[i];
            if(obj==null){
                continue;
            }else{
                if(obj.product_id==item.item_id){
                    positems[obj.trx_id].row.qty = parseFloat(positems[obj.trx_id].row.qty) + 1;
                    delete positems[item_id];
                    flag=0;
                    break;
                }else{
	...
	positems[item_id].order = new Date().getTime();
	localStorage.setItem('positems', JSON.stringify(positems));
        localStorage.setItem('grossitems', JSON.stringify(dt_gross));
        loadItems();
	return true;
}
