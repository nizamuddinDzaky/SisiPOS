...
$('#product_image').bind('change', function() {
 ...
 var file, img;
    if ((file = this.files[0])) {
        img = new Image();
        var maxWidth = <?= $this->Settings->iwidth ?>;
        var maxHeight = <?= $this->Settings->iheight ?>;
         var maxSize = <?= $this->allowed_file_size ?>;
        img.onload = function () {
            if(this.width > maxWidth && this.height > maxHeight){
                alert("Exceed Max Limit");
                $("#InfoImageProduct").html('<i style="color:red;"><sup><strong>*Recomended : </strong><?= "Width:".$this->Settings->iwidth."px, Height:".$this->Settings->iheight."px, File Size:".$this->allowed_file_size ?>Kb</sup></i>');
                    $(".file-input").addClass("file-input-new");
                    $('.class-selector').find(".file-caption-name").empty();
            }else{
               $("#InfoImageProduct").html('<i style="color:green;"><sup><strong>Image Recomended </strong></sup></i>');
            }
 ...
 var file, img;
    if ((file = this.files[0])) {
        ...
        img.onload = function () {
            if(this.width > maxWidth && this.height > maxHeight){
                alert("Exceed Max Limit");
                $("#InfoImageGalery").html('<i style="color:red;"><sup><strong>*Recomended : </strong><?= "Width:".$this->Settings->twidth."px, Height:".$this->Settings->theight."px, Max File Size:".$this->allowed_file_size ?>Kb</sup></i>');
                    $(".file-input").addClass("file-input-new");
                    $('.class-selector-image').find(".file-caption-name").empty();
            }else{
               $("#InfoImageGalery").html('<i style="color:green;"><sup><strong>Image Recomended </strong></sup></i>');
            }
...
<div class="box">
    <div class="box-header">
        ...
        <div class="box-icon">
            <?php echo anchor($mb_add_product, '<i class="icon fa fa-book tip" data-placement="left" title="'.lang("manual_book").'"></i> ', 'target="_blank"') ?>
        </div>
    </div>
            ...
            <div class="col-lg-12">
                ...
                <div class="col-lg-12">
                      <ul class="nav nav-tabs">
                        <li class="active"><a data-toggle="tab" href="#typeproduct_tab"><?= lang("product_type", "type") ?></a></li>
                        <li><a data-toggle="tab" href="#grouping"><?= lang("grouping", "grouping") ?></a></li>
                        <li><a data-toggle="tab" href="#pricing_content"><?= lang("pricing", "pricing") ?></a></li>
                        <li><a data-toggle="tab" href="#setting_content"><?= lang("settings", "settings") ?></a></li>
                      </ul>

                      <div class="tab-content">
                        <!-- Halaman Type Product -->
                        <div id="typeproduct_tab" class="tab-pane fade in active">
                            <div class="row">
                                <div class="col-md-5">
                                    <div class="form-group">
                                        <?= lang("product_type", "type") ?>
                                        <?php
                                        $opts = array('standard' => lang('standard'), 'combo' => lang('combo'), 'digital' => lang('digital'), 'service' => lang('service'), 'consignment'=>lang('consignment'));
                                ...
                                <div class="col-md-6 col-md-offset-1">
                                    <div class="standard consignment">
                                            ...
                                            if (!empty($warehouses)) {
                                                if ($product) {
                                                    ...
                                                    foreach ($warehouses as $warehouse) {
                                                        ...
                                                        echo '<div class="col-md-6 col-sm-6 col-xs-6" style="padding-bottom:15px;">' . $warehouse->name . '<br><div class="form-group">' . form_hidden('wh_' . $warehouse->id, $warehouse->id) . form_input('wh_qty_' . $warehouse->id, (isset($_POST['wh_qty_' . $warehouse->id]) ? $_POST['wh_qty_' . $warehouse->id] : (isset($warehouse->quantity) ? $warehouse->quantity : '')), 'class="form-control wh number-only" id="wh_qty_' . $warehouse->id . '" placeholder="'. lang('products_quantity') .' "') . '</div>';
                                                        if ($Settings->racks) {
                                                            echo '<div class="form-group">' . form_input('rack_' . $warehouse->id, (isset($_POST['rack_' . $warehouse->id]) ? $_POST['rack_' . $warehouse->id] : (isset($warehouse->rack) ? $warehouse->rack : '')), 'class="form-control wh wh_csg" id="rack_' . $warehouse->id . '" placeholder="' . lang('rack') . '"') . '</div>';
                                                        }
                                                        echo '</div>';
                                                    }
                                                    ...
                                                    foreach ($warehouses as $warehouse) {
                                                        //$whs[$warehouse->id] = $warehouse->name;
                                                        echo '<div class="col-md-6 col-sm-6 col-xs-6" style="padding-bottom:15px;">' . $warehouse->name . '<br><div class="form-group">' . form_hidden('wh_' . $warehouse->id, $warehouse->id) . form_input('wh_qty_' . $warehouse->id, (isset($_POST['wh_qty_' . $warehouse->id]) ? $_POST['wh_qty_' . $warehouse->id] : ''), 'class="form-control number-only wh_csg" id="wh_qty_' . $warehouse->id . '" placeholder="'. lang('products') .' ' . lang('quantity') . '"') . '</div>';
                                                        if ($Settings->racks) {
                                                            echo '<div class="form-group">' . form_input('rack_' . $warehouse->id, (isset($_POST['rack_' . $warehouse->id]) ? $_POST['rack_' . $warehouse->id] : ''), 'class="form-control wh_csg" id="rack_' . $warehouse->id . '" placeholder="' . lang('rack') . '"') . '</div>';
                                                        }
                                                        echo '</div>';
                                                    }
                                ...
                                <div class="form-group standard consignment">
                                    <div class="form-group">
                                        <?= lang("supplier", "supplier") ?>
                                        <button type="button" class="btn btn-primary btn-xs standard" id="addSupplier"><i class="fa fa-plus"></i>
                                        </button>
                                    </div>
                                    <div class="row" id="supplier-con">
                                        <div class="col-xs-12">
                                            <div class="form-group">
                                                <?php
                                                echo form_input('supplier', (isset($_POST['supplier']) ? $_POST['supplier'] : ''), 'class="form-control ' . ($product ? '' : 'suppliers') . '" id="' . ($product && ! empty($product->supplier1) ? 'supplier1' : 'supplier') . '" placeholder="' . lang("select") . ' ' . lang("select_supplier") . '" style="width:100%;" required="required"');
                                                ?>
                                            </div>
                                        </div>
                                        <div class="col-xs-6">
                        ...
                        <div id="grouping" class="tab-pane fade">
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="form-group all standard consignment">
                                        <?= lang('product_unit', 'unit'); ?>
                                       <div class="form-group">
                                            <div class="input-group">
                                                <?= form_dropdown('unit', $pu, set_value('unit', ($product ? $product->unit : '')), 'class="form-control tip" id="unit" placeholder="' . lang("select") . " " . lang("unit") . '" required="required" style="width:100%;"'); ?>
                                        
                                                 <?php if ($Owner || $Admin || $LT) { ?>
                                                    <div class="input-group-addon no-print" style="padding: 2px 8px;">
                                                        <a href="<?=site_url('products/add_unit');?>" id="add-unit" class="external" data-toggle="modal" data-target="#myModal"  data-backdrop="static">
                                                            <i class="fa fa-plus-circle" id="addunit" style="font-size: 1.5em;"></i>
                                                        </a>
                                                    </div>
                                                <?php } ?>
                                            </div>
                                            <i style="color:green;"><sup id="InfoUnit"></sup></i>
                                       </div>
                                    </div>
                                    <div class="form-group all standard consignment">
                                        <?= lang('default_sale_unit', 'default_sale_unit'); ?>
                                           <div class="form-group">
                                            <div class="input-group">
                                                ...
                                                <?php if ($Owner || $Admin || $LT) { ?>
                                                <div class="input-group-addon no-print" style="padding: 2px 8px;">
                                                    <a href="<?=site_url('products/add_unit');?>" id="add-sale_unit" class="external" data-toggle="modal" data-target="#myModal"  data-backdrop="static">
                                                        <i class="fa fa-plus-circle" id="addsale_unit" style="font-size: 1.5em;"></i>
                                                    </a>
                                                </div>
                                                <?php } ?>
                                            </div>
                                    ...
                                    <div class="form-group all standard consignment">
                                        <?= lang('default_purchase_unit', 'default_purchase_unit'); ?>
                                          <div class="form-group">
                                            ...
                                            <?php if ($Owner || $Admin || $LT) { ?>
                                                <div class="input-group-addon no-print" style="padding: 2px 8px;">
                                                    <a href="<?=site_url('products/add_unit');?>" id="add-purchase_unit" class="external" data-toggle="modal" data-target="#myModal"  data-backdrop="static">
                                                        <i class="fa fa-plus-circle" id="addpurchase_unit" style="font-size: 1.5em;"></i>
                                                    </a>
                                                </div>
                                            <?php } ?>
                                            </div>
                                ...
                                <div class="col-md-6">
                                    <div class="form-group all">
                                        ...
                                        <div class="form-group">
                                            <div class="input-group">
                                              ...
                                              <?php if ($Owner || $Admin || $LT) { ?>
                                                    <div class="input-group-addon no-print" style="padding: 2px 8px;">
                                                        <a href="<?=site_url('products/add_brand');?>" id="add-customer" class="external" data-toggle="modal" data-target="#myModal"  data-backdrop="static">
                                                            <i class="fa fa-plus-circle" id="addbrand" style="font-size: 1.5em;"></i>
                                                        </a>
                                                    </div>
                                                <?php } ?>
                                    ...
                                    <div class="form-group all">
                                       ...
                                       <div class="form-group">
                                        ...
                                        //  }
                                         echo form_dropdown('category', '', (isset($_POST['category']) ? $_POST['category'] : ($product ? $product->category_id : '')), 'class="form-control select" id="category" placeholder="' . lang("select") . " " . lang("category") . '" required="required" style="width:100%"')
                                        ?>      
                                                 <?php if ($Owner || $Admin || $LT) { ?>
                                                    <div class="input-group-addon no-print" style="padding: 2px 8px;">
                                                        <a href="<?=site_url('products/add_category');?>" id="add-category" class="external" data-toggle="modal" data-target="#myModal"  data-backdrop="static">
                                                            <i class="fa fa-plus-circle" id="addcategory" style="font-size: 1.5em;"></i>
                                                        </a>
                                                    </div>
                                                <?php } ?>
                                            </div>
                                            <i style="color:green;"><sup class="InfoCategory"></sup></i>
                                       </div>
                                    </div>
                                    <div class="form-group all">
                                        ...
                                        <div class="controls" id="subcat_data">
                                            ...
                                            <?php if ($Owner || $Admin || $LT) { ?>
                                                <div class="input-group-addon no-print" style="padding: 2px 8px;">
                                                    <a href="<?=site_url('products/add_category/1');?>" id="add-subcategory" class="external" data-toggle="modal" data-target="#myModal"  data-backdrop="static">
                                                        <i class="fa fa-plus-circle" id="addsubcategory" style="font-size: 1.5em;"></i>
                                                    </a>
                                                </div>
                                            <?php } ?>
                                                </div>
                        ...
                        <!-- End Halaman Groupimg -->

                        <!-- Pricing Content -->
                        <div id="pricing_content" class="tab-pane fade">
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="row">
                                        <div class="form-group standard">
                                            <div class="col-md-6">
                                                ...
                                                <?= form_input('cost', (isset($_POST['cost']) ? $_POST['cost'] : ($product ? $this->sma->formatDecimal($product->cost) : '')), 'class="form-control tip number-only" id="cost" required="required"') ?>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="form-group all">
                                        <div class="row">
                                            <div class="col-md-6">
                                                ...
                                                <?= form_input('price', (isset($_POST['price']) ? $_POST['price'] : ($product ? $this->sma->formatDecimal($product->price) : '')), 'class="form-control tip number-only" id="price" required="required"') ?>
                                                <i class="font-italic text-danger small"><strong><?= lang("example")?> : </strong><?= lang("selling_price") ?> : Rp 200000</i>
                                            </div>
                                            <div class="col-md-6">
                                                <?= lang("credit_price", "price") ?>
                                                <?= form_input('credit_price', (isset($_POST['credit_price']) ? $_POST['credit_price'] : ($product ? $this->sma->formatDecimal($product->price) : '')), 'class="form-control tip number-only" id="credit_price" required="required"') ?>
                                                <i class="font-italic text-danger small"><strong><?= lang("example")?> : </strong><?= lang("credit_price") ?> : Rp 250000</i>
                                            </div>
                        ...
                        <!-- Setting Content -->
                        <div id="setting_content" class="tab-pane fade">
                            <div class="row">
                                <div class="col-md-6">

                                    <div class="form-group all class-selector">
                                        ...
                                        <span id="InfoImageProduct"><i style="color:red;"><sup><strong><?= lang('recomended_1'); ?></strong>
                                            <?= lang('width').":".$this->Settings->iwidth."px, ".lang('height').":".$this->Settings->iheight."px, ".lang('max_file_size').":".$this->allowed_file_size ?>Kb</sup></i></span>
                                    </div>  
                                </div>
                                <div class="col-md-6">
                                     <div class="form-group all class-selector-image">
                                        ...
                                        <span id="InfoImageProduct"><i style="color:red;"><sup><strong><?= lang('recomended_1'); ?></strong>
                                            <?= lang('width').":".$this->Settings->twidth."px, ".lang('height').":".$this->Settings->theight."px, ".lang('max_file_size').":".$this->allowed_file_size ?>Kb</sup></i></span>
                                    </div>
                                    <div id="img-details"></div> 
                                </div>
                                <div class="col-md-6">
                                    <div class="form-group">
                                        <input type="checkbox" class="checkbox" value="1" name="is_retail" id="is_retail" <?= $this->input->post() ? ($this->input->post('is_retail') ? 'checked="checked"' : '') : 'checked="checked"'; ?>>
                                        <label for="is_retail" class="padding05"><?= lang('show_on_aksestoko') ?></label>
                                    </div>
                <!--                    <div class="form-group">
                                        <input type="checkbox" class="checkbox" value="1" name="consign" id="consign" <?= $this->input->post('consign') ? 'checked="checked"' : ''; ?>>
                                        <label for="consign" class="padding05">
                                            <?= lang('consignment'); ?>
                                        </label>
                                    </div>
                                        ...
                                        <div class="row">
                                            <div class="col-md-12">
                                                <div class="well">
                                                    <?php
                                                    if($product){
                                                        foreach($warehouses as $iwh){
                                                            foreach($consignment as $csg){
                                                                if($csg->warehouse_id==$iwh->id){
                                                                    echo '<div class="col-md-3 col-sm-3 col-xs-3">'.$iwh->name.'<div class="form-group">'.form_input('qty_csgm_'.$iwh->id, (isset($_POST['qty_csgm_'.$iwh->id]) ? $_POST['qty_csgm_'.$iwh->id] : ($csg->quantity?$csg->quantity:'0')), 'class="form-control tip number-only" id="qty_csgm_'.$iwh->id.'"').'</div></div>';
                                                                }
                                                            }
                                                        }
                                                    }else{
                                                        foreach($warehouses as $iwh){
                                                            echo '<div class="col-md-3 col-sm-3 col-xs-3">'.$iwh->name.'<div class="form-group">'.form_input('qty_csgm_'.$iwh->id, (isset($_POST['qty_csgm_'.$iwh->id]) ? $_POST['qty_csgm_'.$iwh->id] : '0'), 'class="form-control tip number-only" id="qty_csgm_'.$iwh->id.'"').'</div></div>';
                                                        }
                                                    }?>
                                                    <div class="clearfix"></div>
                                                </div>
                                    ...
                                    <div class="form-group">
                                        <input name="other" type="checkbox" class="checkbox" id="others" value="" <?= isset($_POST['other']) ? 'checked="checked"' : '' ?>/>
                                        <label for="others" class="padding05"><?= lang('additional_settings') ?></label>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-group standard">
                                        ...
                                        <div
                                            class="input-group"> <?= form_input('alert_quantity', (isset($_POST['alert_quantity']) ? $_POST['alert_quantity'] : ($product ? $this->sma->formatQuantity($product->alert_quantity) : '')), 'class="form-control tip number-only" id="alert_quantity"') ?>
                                ...
                                <div class="col-md-12" id="others-con" style="display: none;">
                                    <div class="form-group">
                                    ...
                                    <div id="promo" style="display:none;">
                                        <div class="form-group col-md-4">
                                            ...
                                            <?= form_input('promo_price', set_value('promo_price'), 'class="form-control tip number-only" id="promo_price"'); ?>
                                        </div>
                                        <div class="form-group col-md-4">
                                    ...
                                    <div class="form-group">
                                        <input type="checkbox" class="checkbox" value="1" name="public" id="public" <?= $this->input->post('public') ? 'checked="checked"' : ''; ?>>
                                        <label for="public" class="padding05"><?= lang('show_on_ecomerce') ?></label>
                                    </div>
                                    <div id="extras-public" style="display: none;">
                                        <div class="col-md-4">
                                            <div class="form-group all">
                                                <?= lang('price_online', 'price_online') ?>
                                                <?= form_input('price_online', (isset($_POST['price_online']) ? $_POST['price_online'] : ($product ? $product->price_public : '0')), 'class="form-control tip number-only" id="price_online"') ?>
                                            </div>
                                        </div>
                                        <div class="col-md-4">
                                            <div class="form-group all">
                                                <?= lang('item_weight', 'weight') ?>
                                                <?= form_input('weight', (isset($_POST['weight']) ? $_POST['weight'] : ($product ? $product->weight : '0')), 'class="form-control tip number-only" id="weight"') ?>
                                            </div>
                                        </div>
                                        <div class="col-md-4">
                                            <div class="form-group all">
                                                <?= lang('Min Pembelian', 'minqty') ?>
                                                <?= form_input('minqty', (isset($_POST['minqty']) ? $_POST['minqty'] : ($product ? $product->minqty : '0')), 'class="form-control tip number-only" id="minqty"') ?>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="form-group all">
                                ...
                                <div class="col-md-12">
                                    <div class="form-group">
                                        <?php echo form_submit('add_product', $this->lang->line("add_product"), 'class="btn btn-primary" id="add_product"'); ?>
                                    </div>
                                </div>
...
<script type="text/javascript">
    $(document).ready(function () {
       <?= $_POST['public'] ? '$("#public").iCheck("check"); $("#extras-public").slideDown();': '' ?>
       <?=isset($_POST['cf']) ? '$("#cf").iCheck("check"); $("#extras-con").slideDown();': '' ?>
       <?=isset($_POST['other']) ? '$("#other").iCheck("check"); $("#others-con").slideDown();': '' ?>
       <?=isset($_POST['promotion']) ? '$("#promotion").iCheck("check"); $("#promo").slideDown();': '' ?>     
        ...
        <?=isset($_POST['other']) ? '$("#extras").iCheck("check");': '' ?>
        $('#others').on('ifChecked', function () {
            $('#others-con').slideDown();
        });
        $('#others').on('ifUnchecked', function () {
            $('#others-con').slideUp();
            $("#promotion").iCheck("uncheck");
            $("#extras").iCheck("uncheck");
            $("#public").iCheck("uncheck");
        });

        $('#public').on('ifChecked', function (e) {
               $.ajax({
                type: "POST",
                 data: {  surname: $("#public").val()},
                url: '<?= site_url('products/ecomerce'); ?>',
                    success: function (response) {
                           var data = $.parseJSON(response);
                           if(data.mtid == null){
                               $('#myModal').modal({
                                        show: true,
                                        remote: 'welcome/ecomerce'
                                });
                           }else{
                            $('#extras-public').slideDown();}
                    }
              });
        });
        $('#public').on('ifUnchecked', function (e) {
            $('#extras-public').slideUp();
        });
...
//        $('#consign').on('ifChecked', function (e) {
//            $('#consignment').slideDown();
//        });
//        $('#consign').on('ifUnchecked', function (e) {
//            $('#consignment').slideUp();
//        });
...
//         $("#name").autocomplete({source:['hai','hoole']});
         $("#name").autocomplete({
             source: '<?= site_url('products/suggestions'); ?>',
             minLength: 1,
             autoFocus: false,
             ...
             select: function (event, ui) {
                 ...
                 if (ui.item.id !== 0) {
                        ...
                        $('select[name=barcode_symbology]').val(ui.item.barcode_symbology).change();
                        $('select[name=brand]').val(ui.item.brand).change();
                        $('#category').val(ui.item.category).change();
                        $('#subcategory').val(ui.item.subcategory).change();
                        $('#unit').val(ui.item.unit).change();
                        $('#default_sale_unit').val(ui.item.sale_unit).change();
                        $('#default_purchase_unit').val(ui.item.purchase_unit).change();
                        //alert(JSON.stringify(ui.item));
        //             // $('#code').val(ui.item.code);
        //             // $('#select2-chosen-3').text($( "#type option[value='"+ui.item.type+"']" ).text());
        //             // $('#type').val(ui.item.type);

        //             // $('#select2-chosen-5').text($( "#brand option[value='"+ui.item.brand_code+"']" ).text());
        //             // $('#brand').val(ui.item.brand_code).prop('disabled',true);

        //             // $('#select2-chosen-6').text($( "#category option[value='"+ui.item.category_id+"']" ).text());
        //             // $('#category').val(ui.item.category_id).prop('disabled',true);

        //             // $('#select2-chosen-7').text($( "#unit option[value='"+ui.item.unit_id+"']" ).text());
        //             // $('#unit').val(ui.item.unit_id).prop('disabled',true);

        //             // $('#select2-chosen-8').text($( "#unit option[value='"+ui.item.unit_id+"']" ).text());
        //             // $('#default_sale_unit').val(ui.item.unit_id).prop('disabled',true);

        //             // $('#select2-chosen-9').text($( "#unit option[value='"+ui.item.unit_id+"']" ).text());
        //             // $('#default_purchase_unit').val(ui.item.unit_id).prop('disabled',true);




        //             // $("#brand").val(ui.item.brand).prop('selected',true);
        //             // console.log(ui.item.brand);
            ...
            if (v) {
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        <?= $_POST['default_sale_unit'] ? "$('#default_sale_unit').val('".$_POST['default_sale_unit']."').change();" : "" ?>                      
                        <?= $_POST['default_purchase_unit'] ? "$('#default_purchase_unit').val('".$_POST['default_purchase_unit']."').change();" : "" ?>
...
//        }
    ...
    var tour = {
    id: "guide-products-add",
    onClose: function(){
        complete_guide('products-add');
    },
    onEnd:function(){
        complete_guide('products-add');
    },
    steps: [
        {
            title: "Nama Produk",
            content: "Silahkan isi nama produk",
            target: "name",
            placement: "top"
        },
        {
            title: "Kode Produk",
            content: "Silahkan isi kode produk manual",
            target: "code",
            placement: "top"
        },
        {
            title: "Kode Produk Otomatis",
            content: "Ingin dapat kode produk otomatis? tekan tombol samping kiri",
            target: "random_num",
            placement: "right",
            yOffset: "-20"
        },
        {
            title: "Brand",
            content: "Silahkan pilih brand",
            target: "s2id_brand",
            placement: "top"
        },
        {
            title: "Tambah Brand",
            content: "Tidak menemukan brand yang sesuai? Silahkan tambah brand sesuai keinginanmu",
            target: "addbrand",
            placement: "right",
            yOffset: "-20"
        },
        {
            title: "Kategori",
            content: "Silahkan pilih kategori yang sesuai",
            target: "s2id_category",
            placement: "top"
        },
        {
            title: "Tambah Kategori",
            content: "Tidak menemukan kategori yang sesuai? Silahkan tambah kategori sesuai keinginanmu",
            target: "add-category",
            placement: "right",
            yOffset: "-20"
        },
        {
            title: "Satuan Produk",
            content: "Silahkan pilih satuan produk yang sesuai",
            target: "s2id_kecamatan",
            placement: "top"
        },
        {
            title: "Harga Beli",
            content: "Silahkan isi harga beli",
            target: "cost",
            placement: "top"
        },
        {
            title: "Harga Jual",
            content: "Silahkan isi harga jual",
            target: "price",
            placement: "top"
        },
        {
            title: "Pemasok",
            content: "Silahkan pilih pemasok dari produk yang ingin ditambahkan",
            target: "s2id_supplier",
            placement: "top"
        },
        {
            title: "2 Pemasok Lebih?",
            content: "Memiliki lebih dari 1 pemasok? silahkan tekan tombol +",
            target: "addSupplier",
            placement: "bottom",
            xOffset: "-20"
        }
