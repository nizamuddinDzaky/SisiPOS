...
<div class="modal-dialog modal-lg">
    ...
    <div class="modal-header">
        ...
        <div class="modal-body">
            ...
            <div class="row">
                <div class="col-xs-5">
                    <img id="pr-image" src="<?= url_image_thumb($product->image) ?>"
                    alt="<?= $product->name ?>" class="img-responsive img-thumbnail"/>

                    <div id="multiimages" class="padding10">
                        <?php if (!empty($images)) {
                            echo '<a class="img-thumbnail change_img" href="' .  url_image_thumb($product->image) . '" style="margin-right:5px;"><img class="img-responsive" src="' .  url_image_thumb($product->image) . '" alt="' . url_image_thumb($product->image) . '" style="width:' . $Settings->twidth . 'px; height:' . $Settings->theight . 'px;" /></a>';
                        ...
                        <table class="table table-borderless table-striped dfTable table-right-left">
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td style="width:70%;">
                                        <?php echo $this->sma->save_barcode($product->code, code39, 66, false);
//                                          $this->sma->save_barcode($product->code, $product->barcode_symbology, 66, false); ?><br><br>
                                        <?php // echo $this->sma->qrcode('link', urlencode(site_url('products/view/'.$enc_id.'/'.$enc_company_id)), 2); ?>
                                    </td>
                                </tr>
                                <tr>
                                    <td><?= lang("type"); ?></td>
                                ...
                                <?php if ($product->subcategory_id) { ?>
                                    ...
                                    <?php if ($Owner || $Admin) {
                                        ...
                                        if($product->public){
                                             echo '<tr><td>' . lang("Ecomerce") . '</td><td>' . $this->sma->formatMoney($product->price_public) . ' / '.$product->weight .' Kg</td></tr>';
                                        }
                                    } else {
                            ...
                            <div class="table-responsive">
                                ...
                                <thead>
                                    <tr>
                                        ...
                                        <th><?= lang('quantity') . '<br> (' . lang('rack') . ')'; ?></th>
                                ...
                                <tbody>
                                    <?php foreach ($warehouses as $warehouse) {
                                        ...
                                        if(($this->Admin || $this->Owner) || (!$this->Admin && !$this->Owner && $warehouse->id == $this->session->userdata('warehouse_id')))
                                            echo '<tr><td>' . $warehouse->name . ' (' . $warehouse->code . ')</td>
                                                  <td><strong>' . $this->sma->formatQuantity($warehouse->quantity) . '</strong>' . ($warehouse->rack ? ' (' . $warehouse->rack . ')' : '') . '</td>
                                                  <td><strong>' . $this->sma->formatQuantity($warehouse->quantity_booking) . '</strong>' .'</td></tr>';
                                        // }
                                    } ?>
                                </tbody>
                            </table>
