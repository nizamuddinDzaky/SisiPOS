                ...
                <div class="well well-sm no-print">
                    ...
                    <div class="controls table-controls">
                        ...
                        <div class="form-group">
                            ...
                            <input name="discount" type="checkbox" id="discount" value="1" style="display:inline-block;" />
                            <label for="discount" class="padding05"><?= lang('discount'); ?></label>
                            <input name="bonus" type="checkbox" id="bonus" value="1" style="display:inline-block;" />
                            <label for="bonus" class="padding05"><?= lang('product_bonus'); ?></label>
                        </div>
                            ...
                            if (!empty($barcodes)) {
                                ...
                                foreach ($barcodes as $item) {
                                    for ($r = 1; $r <= $item['quantity']; $r++) {
                                        ...
                                        if($item['discount']){
                                            echo '<div class="vessel">';
                                            echo '<span class="disc1 white-text">'.$item['discount']->discount.'</span>';
                                            if($item['discount']->sub_discount){
                                                echo '<span class="disc2 white-text">+'.$item['discount']->sub_discount.'</span>';                                                
                                            }
                                            echo '</div>';
                                        }
                                        if ($style == 50) {
                                        ...
                                        if($item['name']) {
                                            echo '<span class="barcode_name">'.$item['name'].' '.($item['bonus']?'(+'.$item['bonus']->name.')':'').'</span>';
                                        }
                                        if($item['price']) {
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        $(document).on('ifChecked', '#discount', function(event) {
            localStorage.setItem('bcdiscount', 1);
        });
        $(document).on('ifUnchecked', '#discount', function(event) {
            localStorage.setItem('bcdiscount', 0);
        });
        if (variants = localStorage.getItem('bcdiscount')) {
            if (variants == 1)
        ...
        $(document).on('ifChecked', '#bonus', function(event) {
            localStorage.setItem('bcbonus', 1);
        });
        $(document).on('ifUnchecked', '#bonus', function(event) {
            localStorage.setItem('bcbonus', 0);
        });
        if (variants = localStorage.getItem('bcbonus')) {
            if (variants == 1)
                $('#bonus').iCheck('check');
            else
                $('#bonus').iCheck('uncheck');
        }
        ...
        $('#reset').click(function (e) {
            ...
            bootbox.confirm(lang.r_u_sure, function (result) {
                if (result) {
                    ...
                    if (localStorage.getItem('bcdiscount')) {
                        localStorage.removeItem('bcdiscount');
                    }
                    if (localStorage.getItem('bcbonus')) {
                        localStorage.removeItem('bcbonus');
                    }
