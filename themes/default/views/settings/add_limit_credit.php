...
<script type="text/javascript">
    $(document).ready(function() {
        $('#StCaRData').dataTable({
            ...
            'bProcessing': true,
            'bServerSide': true,
            'sAjaxSource': '<?= site_url('system_settings/getLimitCustomer'. $v) ?>',
            'fnServerData': function(sSource, aoData, fnCallback) {
                ...
                $('#add_filter').click(function() {
                    $('#StCaRDataTable_processing').css('visibility', 'visible');
                    ...
                    for (var i = 0; i < arrayURL.length; i++) {
                        if (arrayURL[i] == 'getLimitCustomer') {
...
<div class="box">
    <div class="box-header">
        <h2 class="blue"><i class="fa-fw fa fa-plus-square-o"></i><?= lang('Add Customer to Limit Credit'); ?> </h2>
        <div class="box-icon">
            <ul class="btn-tasks">
                <li class="dropdown">
    ...
    <div id="form">
        <div class="box-content">
            <div class="row">
                <div class="col-lg-12">
                    <?php echo form_open("system_settings/add_limit_credit/" . $id); ?>
                    <!-- <div id="form"> -->
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <div class="controls">
                                    <select name="provinsi" id="pg-provinsi" id="provinsi" onchange="setProvinsi_pg(this.value,this.options[this.selectedIndex].innerHTML)" class="form-control select" required>
                    ...
                    <div class="form-group">
                        <?php echo form_submit('filter_type', $this->lang->line("submit"), 'id="add_filter" class="btn btn-primary pull-right"'); ?>
                    </div>
                    <!-- </div> -->
                    <?php echo form_close(); ?>
                </div>
...
<div class="row">
    ...
    <div class="col-sm-4">
        <div class="box">
            ...
            <div class="box-content">
                <?php echo form_open("system_settings/save_customer_limit/" . $id); ?>
                <input type="hidden" value="<?= $limit->name ?>" name="limit_name">
                <div style="width: 100%; height:52vh; overflow-y: scroll;">
                    <ul id="list-toko-selected" class="list-group" style="text-align: center; width: 100%; height: 100%; background-color: #dedede;">
                        ...
                        $list_toko_awal = '';
                        if ($customer_limit) {
                            foreach ($customer_limit as $key => $cpg) {
                                $list_toko_awal .= $cpg->id.',';
                                echo '<li id="toko_selected_' . $cpg->id . '" class="list-group-item d-flex justify-content-between align-items-center" style="text-align: left;">' . $cpg->company;
                                echo '<input type="hidden" value="' . $cpg->id . '" name="list_toko[]">';
                                echo '<a href="javascript:void(0)" onclick="unselect_toko(\'toko_selected_' . $cpg->id . '\', \'' . $cpg->custom_id . '\')" class="badge badge-primary badge-pill">';
                        ...
                        ?>
                        <input type="hidden" name="list_toko_awal" value="<?= $list_toko_awal ?>">
                    </ul>
                </div>
                <div class="form-group">
...
<script type="text/javascript">
    var data_cpg = JSON.parse(`<?= json_encode($customer_limit) ?>`);
    $(document).ready(function() {
    ...
    function checkbox_cpg(x) {
        ...
        var result = false;
        for (var i = 0, len = data_cpg.length; i < len; i++) {
            if (data_cpg[i]['id'] === company_id) {
                result = true;
                break;
            }
        }
